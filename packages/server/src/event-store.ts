...
export class EventStore {
  ...
  constructor(dbPath: string) {
    ...
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS agent_pairings (
        ...
        token_hash TEXT NOT NULL UNIQUE,
        created_by TEXT NOT NULL,
        agent_type TEXT NOT NULL CHECK(agent_type IN ('claude-code', 'codex-cli', 'github-copilot', 'llm-api', 'llm-openai-compatible', 'llm-anthropic-compatible')),
  ...
  private migrateAgentPairingAgentTypes(): void {
    ...
    if (!table) return;
    const hasNewConstraint = table.sql.includes("'github-copilot'") && table.sql.includes("'llm-anthropic-compatible'") && table.sql.includes("'codex-cli'") && !table.sql.includes("'codex'") && !table.sql.includes("'opencode'") && !table.sql.includes("'echo'");
    ...
    this.db.exec(`
      ...
      INSERT INTO agent_pairings_next
        ...
        FROM agent_pairings
        WHERE agent_type IN ('claude-code', 'codex-cli', 'github-copilot', 'llm-api', 'llm-openai-compatible', 'llm-anthropic-compatible');
