...
import {
  ...
  type AgentAdapterCompatibility,
  type AttachmentRef,
  type CacpEvent,
...
import {
  ...
  findAnyOpenTurn,
  findOpenTurn,
  findQueuedFollowupMessage,
  findQueuedFollowupMessages,
  hasQueuedFollowup,
...
const CreateRoomSchema = z.object({
  ...
  display_name: z.string().min(1).max(100).default("Owner"),
});
...
const ProposalSchema = z.object({
  ...
  proposal_type: z.string().min(1).max(50),
  policy: PolicySchema,
});
const AgentRegisterSchema = z.object({
  name: z.string().min(1).max(100),
  capabilities: z.array(z.string().max(50)).default([]),
  compatibility: ConnectorCompatibilitySchema,
});
...
const JoinRequestStatusQuerySchema = z.object({
  request_token: z.string().min(1),
});
const JoinRequestListQuerySchema = z.object({
...
const TaskCreateSchema = z.object({
  ...
  mode: z.literal("oneshot").default("oneshot"),
  requires_approval: z.boolean().default(false),
});
const TaskOutputSchema = z.object({
  stream: z.enum(["stdout", "stderr"]),
  chunk: z.string().max(10000),
});
...
const AgentSessionPreviewRequestBodySchema =
  AgentSessionPreviewRequestedPayloadSchema.pick({
    agent_id: true,
    provider: true,
    session_id: true,
  });
const AgentSessionPreviewMessagesBodySchema = z
  .array(AgentSessionPreviewMessagePayloadSchema)
  .min(1)
  .max(50);
const AgentSessionPreviewCompleteBodySchema =
  AgentSessionPreviewCompletedPayloadSchema;
const AgentSessionPreviewFailBodySchema =
  AgentSessionPreviewFailedPayloadSchema;
const AgentSessionImportStartBodySchema =
  AgentSessionImportStartedPayloadSchema;
const AgentSessionImportMessagesBodySchema = z
  .array(AgentSessionImportMessagePayloadSchema)
  .min(1)
  .max(50);
const AgentSessionImportCompleteBodySchema =
  AgentSessionImportCompletedPayloadSchema;
...
export interface LocalAgentLaunchResult {
  pid?: number;
}

export type LocalAgentLauncher = (
  input: LocalAgentLaunchInput
) => Promise<LocalAgentLaunchResult> | LocalAgentLaunchResult;
...
type ProposalState = {
  policy: Policy;
  votes: VoteRecord[];
  terminal_status?: ProposalTerminalStatus;
};
...
type PendingElicitationDecision = {
...
function requireConnectorCompatibility(
  input: unknown,
  provider: AgentType,
  reply: FastifyReply
):
  | {
      manifest: ConnectorCompatibility;
      adapter: AgentAdapterCompatibility;
    }
  | undefined {
  const result = ConnectorCompatibilitySchema.safeParse(input);
  if (!result.success) {
    reply.code(426).send({
  ...
  return { manifest: result.data, adapter };
}

function attachmentRef(attachment: StoredAttachment): AttachmentRef {
...
export function buildLocalAgentConsoleScript(
  input: LocalAgentLaunchInput
): string {
  ...
  return [
    ...
    "Write-Host 'It runs the trusted Local Connector for your local tool-agent room.' -ForegroundColor Cyan",
...
export function buildLocalAgentConsoleSpawnCommand(scriptPath: string): {
  command: string;
  args: string[];
} {
  return {
...
export function defaultLocalAgentLauncher(
  input: LocalAgentLaunchInput
): LocalAgentLaunchResult {
...
export async function buildServer(options: BuildServerOptions = {}) {
  ...
  const dbPath = options.dbPath ?? "cacp.db";
  const store = new EventStore(dbPath);
  const attachmentRoot =
    options.attachmentRoot ??
    (dbPath === ":memory:"
      ? resolve(tmpdir(), `cacp-attachments-${prefixedId("test")}`)
      : resolve(dirname(dbPath), ".cacp", "attachments"));
  ...
  const roomLimiter = new FixedWindowRateLimiter({
    ...
    limit: config.roomCreateLimit,
  });
  const inviteLimiter = new FixedWindowRateLimiter({
    windowMs: config.rateLimitWindowMs,
    limit: config.inviteCreateLimit,
  });
  const joinLimiter = new FixedWindowRateLimiter({
    windowMs: config.rateLimitWindowMs,
    limit: config.joinAttemptLimit,
  });
  const pairingLimiter = new FixedWindowRateLimiter({
    windowMs: config.rateLimitWindowMs,
    limit: config.pairingCreateLimit,
  });
  const pairingClaimLimiter = new FixedWindowRateLimiter({
    windowMs: config.rateLimitWindowMs,
    limit: config.joinAttemptLimit,
  });
  const messageLimiter = new FixedWindowRateLimiter({
    windowMs: config.rateLimitWindowMs,
    limit: config.messageCreateLimit,
  });
  const joinRequestPollLimiter = new FixedWindowRateLimiter({
    windowMs: config.rateLimitWindowMs,
    limit: config.joinAttemptLimit * 2,
  });
  const presenceLimiter = new FixedWindowRateLimiter({
    windowMs: config.rateLimitWindowMs,
    limit: config.presenceChangeLimit,
  });
  const typingLimiter = new FixedWindowRateLimiter({
    windowMs: config.rateLimitWindowMs,
    limit: config.typingEventLimit,
  });
  const orbitLimiter = new FixedWindowRateLimiter({
    windowMs: config.rateLimitWindowMs,
  ...
  function closeParticipantSockets(
    roomId: string,
    participantId: string
  ): void {
  ...
  async function cleanupAbandonedAttachments(): Promise<void> {
    const before = new Date(
      Date.now() - config.attachmentAbandonMs
    ).toISOString();
  ...
  function validateRunMatchesTurn(
    runId: string,
    turnId: string
  ): { ok: true } | { ok: false; error: string; status: number } {
    if (runId !== turnId)
      return { ok: false, error: "run_turn_mismatch", status: 400 };
    return { ok: true };
  }

  function validateOpenAgentRun(
    roomId: string,
    runId: string,
    turnId: string,
    agentId: string,
    ...
    if (participant.role !== "agent" || participant.type !== "agent")
    ...
    if (!openTurn || openTurn.agent_id !== agentId)
      return { ok: false, error: "no_active_turn", status: 403 };
    if (openTurn.turn_id !== runId)
      return { ok: false, error: "turn_not_found", status: 403 };
    return { ok: true };
  }

  function validateTerminalAgentRun(
    roomId: string,
    runId: string,
    turnId: string,
    agentId: string,
    ...
    const openValidation = validateOpenAgentRun(
      roomId,
      runId,
      turnId,
      agentId,
    ...
    if (
      openValidation.error !== "no_active_turn" &&
      openValidation.error !== "turn_not_found"
    )
      return openValidation;

    const turn = findTurnState(roomId, turnId);
    if (!turn || turn.agent_id !== agentId || !turn.terminal_status)
      return openValidation;
    return { ok: true };
  }
  ...
  function providerForAgent(
    ...
    const provider = providerForCapabilities(
      findAgentCapabilities(store.listEvents(roomId), agentId)
    );
    if (
  ...
  function resolveValidatedAgentProvider(
    roomId: string,
    agentId: string,
    requestedProvider: string
  ):
    | { ok: true; provider: LocalAgentProvider }
    | { ok: false; error: string; status: number } {
    const actualProvider = providerForAgent(roomId, agentId);
    if (!actualProvider)
      return {
        ok: false,
        error: "missing_local_agent_capability",
        status: 403,
      };
    if (actualProvider !== requestedProvider)
      return { ok: false, error: "provider_mismatch", status: 403 };
  ...
  function findRunNodeState(
    ...
    for (const storedEvent of store.listEvents(roomId)) {
      if (
        storedEvent.payload.run_id !== runId ||
        storedEvent.payload.node_id !== nodeId
      )
        continue;
        ...
        state = {
          ...
          kind:
            typeof storedEvent.payload.kind === "string"
              ...
              : undefined,
          parent_node_id:
            typeof storedEvent.payload.parent_node_id === "string"
              ? storedEvent.payload.parent_node_id
              : undefined,
          title:
            typeof storedEvent.payload.title === "string"
              ? storedEvent.payload.title
              : undefined,
          detail:
            typeof storedEvent.payload.detail === "object" &&
            storedEvent.payload.detail !== null
              ? (storedEvent.payload.detail as Record<string, unknown>)
              : undefined,
          source_refs:
            typeof storedEvent.payload.source_refs === "object" &&
            storedEvent.payload.source_refs !== null
      ...
      if (!state) continue;
      if (
        storedEvent.type === "agent.run.node.completed" ||
        storedEvent.type === "agent.run.node.failed"
      ) {
        state.terminal = true;
      }
    }
  ...
  function findTerminalRunNodePayload(
    roomId: string,
    runId: string,
    nodeId: string
  ...
  function buildApprovalRequestShape(
    body: z.infer<typeof AgentRunApprovalRequestSchema>
  ...
  function buildElicitationRequestShape(
    body: z.infer<typeof AgentRunElicitationRequestSchema>,
    nodeId: string
  ): Record<string, unknown> {
    return {
  ...
  function validateRunIsOpen(
    roomId: string,
    runId: string
  ):
    | { ok: true; runState: RunState | undefined }
    | { ok: false; error: string; status: number } {
    const runState = findRunState(roomId, runId);
    if (runState?.terminal_status)
      return { ok: false, error: "run_closed", status: 409 };
    return { ok: true, runState };
  }

  function validateNodeIsOpen(
    nodeState: RunNodeState
  ): { ok: true } | { ok: false; error: string; status: number } {
    if (nodeState.terminal)
      return { ok: false, error: "node_closed", status: 409 };
    return { ok: true };
  }

  function createPendingApprovalEntry(
    roomId: string,
    runId: string,
    nodeId: string,
    agentId: string,
    ...
    let resolveDecision:
      ((decision: PendingApprovalDecision) => void) | undefined;
  ...
  function createPendingElicitationEntry(
    roomId: string,
    runId: string,
    nodeId: string,
    agentId: string,
    provider: string,
    requestKey: string
  ): PendingElicitationEntry {
    let resolveDecision:
      ((decision: PendingElicitationDecision) => void) | undefined;
  ...
  function isMatchingApprovalNode(
    state: RunNodeState,
    body: z.infer<typeof AgentRunApprovalRequestSchema>
  ): boolean {
    if (state.kind !== "approval") return false;
    return (
      buildRunNodeRequestKey({
  ...
  function isMatchingElicitationNode(
    state: RunNodeState,
    body: z.infer<typeof AgentRunElicitationRequestSchema>,
    nodeId: string
  ): boolean {
  ...
  function resolvePendingApproval(
    ...
    appendAndPublish(
      event(entry.roomId, "agent.run.node.completed", decision.resolved_by, {
        run_id: entry.runId,
        turn_id: entry.runId,
  ...
  function resolvePendingElicitation(
    key: string,
    decision: PendingElicitationDecision
  ): boolean {
  ...
  function closePendingInteractionsForRun(
    roomId: string,
    runId: string,
    reason: "run_closed"
  ): void {
  ...
  function closePendingInteractionsForRoom(
    roomId: string,
    reason: "run_closed"
  ): void {
    ...
    for (const runId of runIds)
      closePendingInteractionsForRun(roomId, runId, reason);
  }
  ...
  function canViewEvent(
    ...
    if (event.type.startsWith("claude.session_preview.")) {
      if (participant.role === "agent")
        return event.payload.agent_id === participant.id;
      return hasAnyRole(participant, ["owner", "admin"]);
    }
  ...
  function findProposalState(
    ...
    return policy
      ? { policy, votes, terminal_status: terminalStatus }
      : undefined;
  }
  ...
  function findTaskState(
    ...
    for (const storedEvent of store.listEvents(roomId)) {
      if (
        storedEvent.type === "task.created" &&
        storedEvent.payload.task_id === taskId &&
        typeof storedEvent.payload.target_agent_id === "string"
      ) {
        task = {
          target_agent_id: storedEvent.payload.target_agent_id,
          started: false,
        };
        continue;
      }
  ...
  function findTurnState(
    ...
    for (const storedEvent of store.listEvents(roomId)) {
      ...
      if (storedEvent.type === "agent.turn.completed")
        turn.terminal_status = "completed";
      if (storedEvent.type === "agent.turn.failed")
        turn.terminal_status = "failed";
    }
    return turn;
  }
  ...
  function isAgentOnline(events: CacpEvent[], agentId: string): boolean {
    for (const storedEvent of [...events].reverse()) {
      if (
        storedEvent.type === "agent.status_changed" &&
  ...
  function requireAssignedAgentTask(
    roomId: string,
    taskId: string,
    participant: Participant,
    reply: FastifyReply
  ...
  function requireAssignedAgentTurn(
    roomId: string,
    turnId: string,
    participant: Participant,
    reply: FastifyReply
  ...
  function findLastTurnId(events: CacpEvent[]): string | undefined {
    for (let i = events.length - 1; i >= 0; i--) {
      ...
      if (
        ...
        typeof e.payload.turn_id === "string"
      ) {
  ...
  function claudeQueuedFollowupDetails(
    messages: ConversationMessage[],
    names: Map<string, string>,
    roles: Map<string, string>
  ):
    | { messageText: string; speakerName: string; speakerRole: string }
    | undefined {
    ...
    return {
      messageText: [
        ...
        ...messages.map(
          (message) =>
            `${names.get(message.actor_id) ?? message.actor_id} (${roles.get(message.actor_id) ?? "member"}): ${message.text}`
        ),
      ].join("\n"),
  ...
  function createMainInputTurnRequestEvents(
    roomId: string,
    input: {
      actorId: string;
      authorName: string;
  ...
  function createAgentTurnRequestEvents(
    roomId: string,
    actorId: string,
    reason: "human_message" | "queued_followup",
    ...
    if (!isAgentOnline(events, activeAgentId)) {
      ...
      return openTurn
        ? [
            event(roomId, "agent.turn.failed", actorId, {
              turn_id: openTurn.turn_id,
              agent_id: activeAgentId,
              error: "active_agent_offline",
            }),
          ]
        : [];
    }
    ...
    if (openTurn) {
      ...
      return [
        event(roomId, "agent.turn.followup_queued", actorId, {
          turn_id: openTurn.turn_id,
          agent_id: openTurn.agent_id,
        }),
      ];
    }
    const turnId = prefixedId("turn");
    ...
    if (localProvider) {
      ...
      const roles = new Map(
        participants.map((participant) => [participant.id, participant.role])
      );
      let messageText = "";
      let speakerName = "";
      let speakerRole = "";
      ...
      if (reason === "human_message") {
        ...
        if (latestMessage) {
          ...
          speakerName =
            names.get(latestMessage.actor_id) ?? latestMessage.actor_id;
          speakerRole = roles.get(latestMessage.actor_id) ?? "member";
        }
      } else if (reason === "queued_followup") {
        const queuedMessages = previousTurnId
          ? findQueuedFollowupMessages(events, previousTurnId)
          : [];
        const queuedDetails = claudeQueuedFollowupDetails(
          queuedMessages.length > 0
            ? queuedMessages
            : previousTurnId
              ? [findQueuedFollowupMessage(events, previousTurnId)].filter(
      ...
      return [
        event(roomId, "agent.turn.requested", actorId, {
          ...
          mode,
          source: "composer",
          content: { text: messageText, attachments: [] },
        }),
      ];
    }
    return [];
  ...
  function validateClaudeAgent(
    ...
    if (activeAgentId !== agentId)
      return { ok: false, error: "not_active_agent", status: 403 };
  ...
  function validateLocalAgentProvider(
    roomId: string,
    agentId: string,
    provider: LocalAgentProvider
  ): { ok: true } | { ok: false; error: string; status: number } {
    const target = findParticipant(roomId, agentId);
    if (!target || target.type !== "agent" || target.role !== "agent")
  ...
  function validateLocalAgentRuntime(
    roomId: string,
    agentId: string,
    provider: LocalAgentProvider,
    turnId: string
    ...
    if (!openTurn || openTurn.agent_id !== agentId)
      return { ok: false, error: "no_active_turn", status: 403 };
    if (openTurn.turn_id !== turnId)
      return { ok: false, error: "turn_not_found", status: 403 };
    return { ok: true };
  }

  function validateClaudeRuntime(
    roomId: string,
    agentId: string,
    turnId: string
  ): { ok: true } | { ok: false; error: string; status: number } {
  ...
  function validateTurnBelongsToAgent(
    events: CacpEvent[],
    turnId: string,
    agentId: string
  ): boolean {
  ...
  function hasClaudeSessionReady(
    events: CacpEvent[],
    agentId: string
  ): boolean {
    let selectionIndex = -1;
    let selection:
      { mode: "fresh" } | { mode: "resume"; session_id: string } | undefined;
    for (let index = events.length - 1; index >= 0; index -= 1) {
      const storedEvent = events[index];
      if (
        storedEvent.type !== "claude.session_selected" ||
        storedEvent.payload.agent_id !== agentId
      )
        continue;
      ...
      if (
        storedEvent.payload.mode === "resume" &&
        typeof storedEvent.payload.session_id === "string"
      ) {
        selectionIndex = index;
        selection = {
          mode: "resume",
          session_id: storedEvent.payload.session_id,
        };
        break;
      }
    }
    ...
    return events.slice(selectionIndex + 1).some((storedEvent) => {
      if (
        storedEvent.type !== "claude.session_ready" ||
        storedEvent.payload.agent_id !== agentId
      )
        return false;
      if (selection.mode === "fresh")
        return storedEvent.payload.mode === "fresh";
      return (
        storedEvent.payload.mode === "resume" &&
        storedEvent.payload.session_id === selection.session_id
      );
    });
  }

  function latestClaudeSessionSelection(
    events: CacpEvent[],
    agentId: string
  ): { mode: "fresh" } | { mode: "resume"; session_id: string } | undefined {
    for (const storedEvent of [...events].reverse()) {
  ...
  function latestLocalAgentSessionSelection(
    events: CacpEvent[],
    agentId: string,
    provider: LocalAgentProvider
  ): { mode: "fresh" } | { mode: "resume"; session_id: string } | undefined {
    for (const storedEvent of [...events].reverse()) {
      if (
        storedEvent.type !== "agent.session_selected" ||
        storedEvent.payload.agent_id !== agentId ||
        storedEvent.payload.provider !== provider
      )
        continue;
      if (storedEvent.payload.mode === "fresh") return { mode: "fresh" };
      if (
  ...
  function hasLocalAgentSessionReady(
    events: CacpEvent[],
    agentId: string,
    provider: LocalAgentProvider
  ): boolean {
    ...
    return events.slice(selectionIndex + 1).some((storedEvent) => {
      if (
        storedEvent.type !== "agent.session_ready" ||
        storedEvent.payload.agent_id !== agentId ||
        storedEvent.payload.provider !== provider
      )
        return false;
  ...
  function validateSelectedLocalAgentResumeSession(
    roomId: string,
    agentId: string,
    provider: LocalAgentProvider,
    sessionId: string
  ): { ok: true } | { ok: false; error: string; status: number } {
    const selection = latestLocalAgentSessionSelection(
      store.listEvents(roomId),
      agentId,
      provider
    );
    if (!selection || selection.mode !== "resume") {
      return {
        ok: false,
        error: "agent_resume_session_not_selected",
        status: 409,
      };
    }
  ...
  function validateSelectedLocalAgentSessionReady(
    roomId: string,
    agentId: string,
    provider: LocalAgentProvider,
    body: { mode: "fresh" } | { mode: "resume"; session_id: string }
  ): { ok: true } | { ok: false; error: string; status: number } {
    const selection = latestLocalAgentSessionSelection(
      store.listEvents(roomId),
      agentId,
    ...
    if (!selection)
      return { ok: false, error: "agent_session_not_selected", status: 409 };
    if (body.mode !== selection.mode)
      return {
        ok: false,
        error: "agent_session_selection_mismatch",
        status: 409,
      };
    if (
      body.mode === "resume" &&
      selection.mode === "resume" &&
      body.session_id !== selection.session_id
    ) {
      return {
        ok: false,
        error: "agent_session_selection_mismatch",
  ...
  function validateSelectedClaudeResumeSession(
    roomId: string,
    agentId: string,
    sessionId: string
  ): { ok: true } | { ok: false; error: string; status: number } {
    const selection = latestClaudeSessionSelection(
      store.listEvents(roomId),
      agentId
    );
    if (!selection || selection.mode !== "resume") {
  ...
  function validateSelectedClaudeSessionReady(
    roomId: string,
    agentId: string,
    body: { mode: "fresh" } | { mode: "resume"; session_id: string }
  ): { ok: true } | { ok: false; error: string; status: number } {
    ...
    if (!selection)
      return { ok: false, error: "claude_session_not_selected", status: 409 };
    if (body.mode !== selection.mode)
      return {
        ok: false,
        error: "claude_session_selection_mismatch",
        status: 409,
      };
    if (
      body.mode === "resume" &&
  ...
  function validateImportMessageBatch(
    roomId: string,
    importId: string,
    messages: Array<{ agent_id: string; session_id: string; sequence: number }>
  ): { ok: true } | { ok: false; error: string; status: number } {
    const related = claudeImportEvents(roomId, importId);
    const started = related.find(
      (storedEvent) => storedEvent.type === "claude.session_import.started"
    );
    if (!started) return { ok: false, error: "unknown_import", status: 404 };
    if (
      related.some(
        (storedEvent) =>
          storedEvent.type === "claude.session_import.completed" ||
          storedEvent.type === "claude.session_import.failed"
      )
    ) {
    ...
    if (
      !messages.every(
        (message) =>
          message.agent_id === started.payload.agent_id &&
          message.session_id === started.payload.session_id
      )
    ) {
      return { ok: false, error: "import_session_mismatch", status: 400 };
    }
    const existingCount = related.filter(
      (storedEvent) => storedEvent.type === "claude.session_import.message"
    ).length;
    const expectedSequences = messages.map((_, index) => existingCount + index);
    if (
      !messages.every(
        (message, index) => message.sequence === expectedSequences[index]
      )
    ) {
  ...
  function validateImportComplete(
    roomId: string,
    importId: string,
    body: {
      agent_id: string;
    ...
    if (
      body.agent_id !== started.payload.agent_id ||
      body.session_id !== started.payload.session_id
    ) {
      return { ok: false, error: "import_session_mismatch", status: 400 };
    }
    const expectedCount =
      typeof started.payload.message_count === "number"
        ? started.payload.message_count
        : undefined;
    const uploaded = related.filter(
      (storedEvent) => storedEvent.type === "claude.session_import.message"
    );
    const sequenceSet = new Set(
      uploaded.map((storedEvent) => storedEvent.payload.sequence)
    );
    const hasContinuousSequence =
      uploaded.every(
        (storedEvent) => typeof storedEvent.payload.sequence === "number"
      ) &&
      Array.from({ length: uploaded.length }, (_, index) => index).every(
        (sequence) => sequenceSet.has(sequence)
      );
    if (
      body.imported_message_count !== uploaded.length ||
      expectedCount !== uploaded.length ||
  ...
  function validatePreviewOpen(
    ...
    const requested = related.find(
      (storedEvent) => storedEvent.type === "claude.session_preview.requested"
    );
    if (!requested) return { ok: false, error: "unknown_preview", status: 404 };
    if (
      requested.payload.agent_id !== agentId ||
      requested.payload.session_id !== sessionId
    ) {
    ...
    if (
      related.some(
        (storedEvent) =>
          storedEvent.type === "claude.session_preview.completed" ||
          storedEvent.type === "claude.session_preview.failed"
      )
    ) {
  ...
  function validatePreviewComplete(
    roomId: string,
    previewId: string,
    body: {
      agent_id: string;
    ...
    if (!requested) return { ok: false, error: "unknown_preview", status: 404 };
    if (
      requested.payload.agent_id !== body.agent_id ||
      requested.payload.session_id !== body.session_id
    ) {
      return { ok: false, error: "preview_session_mismatch", status: 400 };
    }
    const uploaded = related.filter(
      (storedEvent) => storedEvent.type === "claude.session_preview.message"
    );
    const sequenceSet = new Set(
      uploaded.map((storedEvent) => storedEvent.payload.sequence)
    );
    ...
    if (
      body.previewed_message_count !== uploaded.length ||
      !hasContinuousSequence
    ) {
  ...
  function validateAgentImportMessageBatch(
    roomId: string,
    importId: string,
    messages: Array<{
      agent_id: string;
    ...
    const started = related.find(
      (storedEvent) => storedEvent.type === "agent.session_import.started"
    );
    if (!started) return { ok: false, error: "unknown_import", status: 404 };
    if (
      related.some(
        (storedEvent) =>
          storedEvent.type === "agent.session_import.completed" ||
          storedEvent.type === "agent.session_import.failed"
      )
    ) {
      return { ok: false, error: "import_closed", status: 409 };
    }
    if (
      !messages.every(
        (message) =>
          message.agent_id === started.payload.agent_id &&
          message.provider === started.payload.provider &&
          message.session_id === started.payload.session_id
      )
    ) {
      return { ok: false, error: "import_session_mismatch", status: 400 };
    ...
    const existingCount = related.filter(
      (storedEvent) => storedEvent.type === "agent.session_import.message"
    ).length;
    const expectedSequences = messages.map((_, index) => existingCount + index);
    if (
      !messages.every(
  ...
  function validateAgentImportComplete(
    roomId: string,
    importId: string,
    body: {
      agent_id: string;
    ...
    if (
      body.agent_id !== started.payload.agent_id ||
      body.provider !== started.payload.provider ||
      body.session_id !== started.payload.session_id
    ) {
      return { ok: false, error: "import_session_mismatch", status: 400 };
    }
  ...
  function validateAgentPreviewOpen(
    roomId: string,
    previewId: string,
    agentId: string,
    provider: string,
    ...
    const requested = related.find(
      (storedEvent) => storedEvent.type === "agent.session_preview.requested"
    );
    if (!requested) return { ok: false, error: "unknown_preview", status: 404 };
    if (
      requested.payload.agent_id !== agentId ||
      requested.payload.provider !== provider ||
      requested.payload.session_id !== sessionId
    ) {
      return { ok: false, error: "preview_session_mismatch", status: 400 };
    }
    if (
      related.some(
        (storedEvent) =>
          storedEvent.type === "agent.session_preview.completed" ||
          storedEvent.type === "agent.session_preview.failed"
      )
    ) {
      return { ok: false, error: "preview_closed", status: 409 };
    }
  ...
  function validateAgentPreviewComplete(
    roomId: string,
    previewId: string,
    body: {
      agent_id: string;
    ...
    if (
      requested.payload.agent_id !== body.agent_id ||
      requested.payload.provider !== body.provider ||
      requested.payload.session_id !== body.session_id
    ) {
      return { ok: false, error: "preview_session_mismatch", status: 400 };
    }
    const uploaded = related.filter(
      (storedEvent) => storedEvent.type === "agent.session_preview.message"
    );
    const sequenceSet = new Set(
      uploaded.map((storedEvent) => storedEvent.payload.sequence)
    );
  ...
  function createStoredAgentPairing(
    roomId: string,
    actorId: string,
    body: z.infer<typeof AgentPairingCreateSchema>
  ) {
    ...
    const storedEvents = store.transaction(() => {
      ...
      return [
        store.appendEvent(
          event(roomId, "agent.pairing_created", actorId, {
            pairing_id: pairingId,
            agent_type: body.agent_type,
            permission_level: body.permission_level,
            expires_at: expiresAt,
      ...
      try {
        const part = await (
          request as typeof request & {
            file: (options?: FastifyMultipartBaseOptions) => Promise<
              | {
          ...
          limits: { files: 1, fileSize: config.maxAttachmentBytes, fields: 0 },
        });
        if (!part) return deny(reply, "attachment_required", 400);
        staged = await attachmentStore.stage(
          part.file,
  ...
  app.post("/rooms", async (request, reply) => {
    ...
    const storedEvents = store.transaction(() => {
      ...
      const owner = store.addParticipant({
        room_id: roomId,
        id: ownerId,
        token: ownerToken,
        display_name: body.display_name,
      ...
      return [
        store.appendEvent(
          event(roomId, "room.created", ownerId, {
            name: body.name,
            created_by: ownerId,
          })
        ),
        store.appendEvent(
          event(roomId, "participant.joined", ownerId, {
            participant: publicParticipant(owner),
          })
        ),
      ];
    });
    ...
    return reply
      .code(201)
      .send({ room_id: roomId, owner_id: ownerId, owner_token: ownerToken });
  });
  ...
  app.get<{ Params: { roomId: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!participant) {
        ...
        const revoked = token
          ? store.getRevokedParticipantByToken(request.params.roomId, token)
          : undefined;
  ...
  app.post<{ Params: { roomId: string } }>(
    ...
    async (request, reply) => {
      ...
      return reply
        .code(201)
        .send({ ok: true, event_type: "participant.presence_changed" });
    }
  );

  app.post<{ Params: { roomId: string } }>(
    "/rooms/:roomId/activity/typing/start",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      return reply
        .code(201)
        .send({ ok: true, event_type: "participant.typing_started" });
    }
  );

  app.post<{ Params: { roomId: string } }>(
    "/rooms/:roomId/activity/typing/stop",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      return reply
        .code(201)
        .send({ ok: true, event_type: "participant.typing_stopped" });
    }
  );
  ...
  app.get<{ Params: { roomId: string }; Querystring: { token?: string } }>(
    ...
    (socket, request) => {
      ...
      if (participant.role === "agent") {
        appendAndPublish(
          event(roomId, "agent.status_changed", participant.id, {
      ...
      for (const existingEvent of store.listEvents(roomId)) {
        if (canViewEvent(existingEvent, participant))
          socket.send(JSON.stringify(existingEvent));
      }
      ...
      if (HUMAN_ROLES.includes(participant.role)) {
        ...
        for (const synthetic of orbit.replayFor(participant)) {
          ...
          const noteId =
            typeof payload.note_id === "string" ? payload.note_id : undefined;
          const createdAt =
      ...
      const unsubscribe = bus.subscribe(roomId, (envelope) => {
        if (
          canDeliverEnvelope(envelope, participant) &&
          canViewEvent(envelope.event, participant)
        ) {
  ...
  app.post<{ Params: { roomId: string } }>(
    ...
    async (request, reply) => {
      ...
      const humans = store
        .getParticipants(request.params.roomId)
        .filter((p) => p.role !== "agent");
      const remainingSlots = config.maxParticipantsPerRoom - humans.length;
      if (remainingSlots <= 0)
        return deny(reply, "max_participants_reached", 409);
      const maxUses = Math.min(body.max_uses, remainingSlots);
      const historyAccess =
        body.main_thread_history_access ??
        (body.role === "observer" ? "denied" : "allowed");
      const storedEvents = store.transaction(() => {
        store.createInvite({
          invite_id: inviteId,
          room_id: request.params.roomId,
        ...
        return [
          store.appendEvent(
            event(request.params.roomId, "invite.created", participant.id, {
              invite_id: inviteId,
              role: body.role,
              main_thread_history_access: historyAccess,
              expires_at: expiresAt,
  ...
  app.get<{ Querystring: { token: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!token || typeof token !== "string")
        return deny(reply, "missing_token", 400);
      const invite = store.getInviteByTokenHash(
        hashToken(token, config.tokenSecret)
      );
      if (!invite)
      ...
      const pendingCount = store.countPendingJoinRequestsByInvite(
        invite.invite_id
      );
      if (
        invite.max_uses !== null &&
        invite.used_count + pendingCount >= invite.max_uses
      ) {
  ...
  app.post<{ Params: { roomId: string } }>(
    ...
    async (request, reply) => {
      ...
      const result = store.transaction(() => {
        const invite = store.getInviteByTokenHash(
          hashToken(body.invite_token, config.tokenSecret)
        );
        if (!invite || invite.room_id !== roomId)
          return { ok: false as const, error: "invalid_invite" };
        if (invite.revoked_at !== null)
          return { ok: false as const, error: "invite_revoked", status: 409 };
        if (Date.parse(invite.expires_at) <= Date.now())
        ...
        if (
          ...
          return {
            ok: false as const,
            error: "invite_use_limit_reached",
            status: 409,
          };
      ...
      return reply.code(201).send({
        ...
        status: "pending",
        main_thread_history_access: result.stored!.main_thread_history_access,
        expires_at: expiresAt,
      });
    }
  );
  ...
  app.get<{
    ...
    Querystring: { request_token?: string };
  }>("/rooms/:roomId/join-requests/:requestId", async (request, reply) => {
    ...
    if (
      !current ||
      current.room_id !== request.params.roomId ||
      current.request_token_hash !== tokenHash
    )
      return deny(reply, "unknown_join_request", 404);
    if (
      current.status === "pending" &&
      Date.parse(current.expires_at) <= Date.now()
    ) {
      const expired = store.expireJoinRequest(
        current.request_id,
    ...
    if (current.status === "approved") {
      return {
        ...
        participant_token: current.participant_token_sealed
          ? openSecret(current.participant_token_sealed, config.tokenSecret)
          : undefined,
        role: current.role,
  ...
  app.get<{ Params: { roomId: string }; Querystring: { status?: string } }>(
    ...
    async (request, reply) => {
      ...
      return {
        requests: store
          .listJoinRequests(request.params.roomId, query.status)
  ...
  app.post<{ Params: { roomId: string; requestId: string } }>(
    ...
    async (request, reply) => {
      ...
      const result = store.transaction(() => {
        const current = store.getJoinRequest(request.params.requestId);
        if (!current || current.room_id !== request.params.roomId)
          return {
            ok: false as const,
            error: "unknown_join_request",
            status: 404,
          };
        ...
        const humans = store
          .getParticipants(current.room_id)
          .filter((p) => p.role !== "agent");
        if (humans.length >= config.maxParticipantsPerRoom)
          return {
            ok: false as const,
        ...
        try {
          ...
          if (message === "invite_use_limit_reached")
            return {
              ok: false as const,
              error: "invite_use_limit_reached",
              status: 409,
        ...
        const joined = store.addParticipant({
          room_id: current.room_id,
          id: participantId,
          token: participantToken,
          display_name: current.display_name,
          type: role === "observer" ? "observer" : "human",
          role,
          main_thread_history_access: current.main_thread_history_access,
        });
        const approved = store.approveJoinRequest(current.request_id, {
          ...
          participant_token_sealed: sealSecret(
            participantToken,
            config.tokenSecret
          ),
        });
        const events: CacpEvent[] = [
          store.appendEvent(
            event(current.room_id, "join_request.approved", participant.id, {
              ...publicJoinRequest(approved),
              invite_id: current.invite_id,
            })
          ),
          store.appendEvent(
            event(current.room_id, "participant.joined", joined.id, {
              participant: publicParticipant(joined),
            })
          ),
        ];
        ...
        if (
          inviteAfter &&
          inviteAfter.max_uses !== null &&
          inviteAfter.used_count >= inviteAfter.max_uses &&
          inviteAfter.revoked_at === null
        ) {
          const revoked = store.revokeInvite(current.invite_id, decidedAt);
          events.push(
            store.appendEvent(
              event(current.room_id, "invite.revoked", participant.id, {
                invite_id: current.invite_id,
      ...
      return reply
        .code(201)
        .send({ participant_id: result.participant.id, role: result.role });
    }
  );
  ...
  app.post<{ Params: { roomId: string; requestId: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!current || current.room_id !== request.params.roomId)
        return deny(reply, "unknown_join_request", 404);
      const rejected = store.rejectJoinRequest(
    ...
    async (request, reply) => {
      ...
      const storedEvents = store.transaction(() => {
        ...
        for (const target of participants) {
          ...
          events.push(
            store.appendEvent(
              event(request.params.roomId, "participant.removed", actor.id, {
                participant_id: target.id,
                removed_by: actor.id,
                removed_at: removedAt,
            ...
            events.push(
              store.appendEvent(
                event(
                  ...
                  target.id,
                  { agent_id: target.id, status: "offline" }
                )
              )
            );
          }
  ...
  app.post<{ Params: { roomId: string; participantId: string } }>(
    "/rooms/:roomId/participants/:participantId/remove",
    async (request, reply) => {
      const actor = requireParticipant(store, request.params.roomId, request);
      if (!actor) return deny(reply, "invalid_token");
      if (!hasHumanRole(actor, ["owner", "admin"]))
        return deny(reply, "forbidden", 403);
      const body = z
      ...
      if (target.role === "owner")
        return deny(reply, "cannot_remove_owner", 409);
      if (target.id === actor.id) return deny(reply, "cannot_remove_self", 409);
      if (actor.role === "admin" && target.role === "admin")
        return deny(reply, "cannot_remove_admin", 409);
      const removedAt = new Date().toISOString();
      const storedEvents = store.transaction(() => {
        store.revokeParticipant(
          request.params.roomId,
        ...
        if (target.role === "agent") {
          events.push(
            store.appendEvent(
              event(request.params.roomId, "agent.status_changed", target.id, {
                agent_id: target.id,
                status: "offline",
              })
            )
          ...
          if (
            findActiveAgentId(store.listEvents(request.params.roomId)) ===
            target.id
          ) {
            events.push(
              store.appendEvent(
                event(request.params.roomId, "room.agent_selected", actor.id, {
                  agent_id: "",
                })
              )
            );
  ...
  app.post<{ Params: { roomId: string; participantId: string } }>(
    ...
    async (request, reply) => {
      ...
      if (target.id === actor.id)
        return deny(reply, "cannot_change_own_role", 409);
      if (target.role === "owner")
        return deny(reply, "cannot_change_owner_role", 409);
      const updatedAt = new Date().toISOString();
      const storedEvents = store.transaction(() => {
        store.updateParticipantRole(
          request.params.roomId,
          target.id,
          body.role
        );
        return [
          store.appendEvent(
            event(request.params.roomId, "participant.role_updated", actor.id, {
              participant_id: target.id,
  ...
  app.post<{ Params: { roomId: string } }>(
    ...
    async (request, reply) => {
      ...
      const body = z
        .object({
          text: z.string().trim().min(1).max(config.maxMessageLength),
          attachment_ids: z
      ...
      const attachments = store.getAttachments(roomId, attachmentIds);
      if (
      ...
      if (!openTurn) {
        ...
        triggerTurnId =
          (
            turnRequestEvents.find((e) => e.type === "agent.turn.requested")
              ?.payload as { turn_id: string }
          )?.turn_id ?? "";
      ...
      return reply
        .code(201)
        .send({ input_id: inputId, status: openTurn ? "queued" : "triggered" });
    }
  );

  app.post<{ Params: { roomId: string; inputId: string } }>(
    "/rooms/:roomId/main-inputs/:inputId/cancel",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      appendAndPublish(
        event(roomId, "main_input.cancelled", participant.id, {
          input_id: inputId,
          message_id: inputId,
  ...
  app.post<{ Params: { roomId: string; noteId: string } }>(
    "/rooms/:roomId/orbit/notes/:noteId/like",
    async (request, reply) => {
      if (!orbitLimiter.allow(request.ip)) return tooMany(reply);
      const participant = requireParticipant(
        store,
      ...
      if (note.author_id === participant.id)
        return deny(reply, "self_like_not_allowed", 409);
      const result = orbit.setLike(request.params.noteId, participant.id, true);
      publishRoleFiltered(
        event(roomId, "orbit.like.changed", participant.id, {
  ...
  app.post<{ Params: { roomId: string } }>(
    ...
    async (request, reply) => {
      ...
      const speakerRole = (lastNote?.author_role ??
      ...
      if (quotedNoteIds.length > 0) {
        publishRoleFiltered(
          event(roomId, "orbit.notes.quoted", participant.id, {
            note_ids: quotedNoteIds,
          }),
          HUMAN_ROLES
        );
      }
  ...
  app.post<{ Params: { roomId: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!hasHumanRole(participant, ["owner", "admin"]))
        return deny(reply, "forbidden", 403);
      const body = z
        .object({ text: z.string().min(1).max(config.maxMessageLength) })
        .parse(request.body);
      const storedEvents = store.transaction(() => {
        const messageId = prefixedId("msg");
        const message = store.appendEvent(
          event(request.params.roomId, "message.created", participant.id, {
            message_id: messageId,
            content: { text: body.text, attachments: [] },
            kind: "human",
            created_at: new Date().toISOString(),
          })
        );
        ...
        return [
          message,
          ...createAgentTurnRequestEvents(
            request.params.roomId,
            participant.id,
            "human_message"
          ).map((nextEvent) => store.appendEvent(nextEvent)),
        ];
      });
      publishEvents(storedEvents);
  ...
  app.post<{ Params: { roomId: string } }>(
    ...
    async (request, reply) => {
      ...
      appendAndPublish(
        event(request.params.roomId, "proposal.created", participant.id, {
  ...
  app.post<{ Params: { roomId: string; proposalId: string } }>(
    ...
    async (request, reply) => {
      ...
      const evaluation = evaluatePolicy(
        state.policy,
        store.getParticipants(request.params.roomId),
        votes
      );
      const eventsToAppend = [
        event(request.params.roomId, "proposal.vote_cast", participant.id, {
      ...
      if (evaluation.status === "approved")
        eventsToAppend.push(
          event(request.params.roomId, "proposal.approved", participant.id, {
            proposal_id: request.params.proposalId,
            evaluation,
          })
        );
      if (evaluation.status === "rejected")
        eventsToAppend.push(
          event(request.params.roomId, "proposal.rejected", participant.id, {
            proposal_id: request.params.proposalId,
            evaluation,
          })
        );
      if (evaluation.status === "expired")
        eventsToAppend.push(
          event(request.params.roomId, "proposal.expired", participant.id, {
            proposal_id: request.params.proposalId,
            evaluation,
          })
        );
      const storedEvents = store.transaction(() =>
        eventsToAppend.map((nextEvent) => store.appendEvent(nextEvent))
      );
      publishEvents(storedEvents);
  ...
  app.post<{ Params: { roomId: string } }>(
    ...
    async (request, reply) => {
      ...
      const agents = store
        .getParticipants(request.params.roomId)
        .filter((p) => p.role === "agent");
      if (agents.length >= config.maxAgentsPerRoom)
        return deny(reply, "max_agents_reached", 409);
      const rawBody = request.body as {
        capabilities?: unknown;
        compatibility?: unknown;
      };
      const rawCapabilities = Array.isArray(rawBody?.capabilities)
        ? rawBody.capabilities.filter(
            (entry): entry is string => typeof entry === "string"
          )
        : [];
      const provider = providerForCapabilities(rawCapabilities);
      if (!provider) return deny(reply, "unsupported_agent_provider", 400);
      const compatibility = requireConnectorCompatibility(
        rawBody?.compatibility,
        provider,
        reply
      );
      ...
      const storedEvents = store.transaction(() => {
        const added = store.addParticipant({
          room_id: request.params.roomId,
          id: agentId,
          token: agentToken,
        ...
        return [
          store.appendEvent(
            event(request.params.roomId, "agent.registered", participant.id, {
              agent_id: agentId,
              name: body.name,
              capabilities: body.capabilities,
              adapter: compatibility.adapter,
              connector_version: compatibility.manifest.connector_version,
            })
          ),
          store.appendEvent(
            event(request.params.roomId, "participant.joined", agentId, {
              participant: publicParticipant(added),
            })
          ),
        ];
      });
      ...
      return reply
        .code(201)
        .send({ agent_id: agentId, agent_token: agentToken });
    }
  );

  app.post<{ Params: { roomId: string } }>(
    "/rooms/:roomId/agent-pairings",
    async (request, reply) => {
  ...
  app.post<{ Params: { roomId: string } }>(
    "/rooms/:roomId/agent-pairings/start-local",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (!hasHumanRole(participant, ["owner", "admin"]))
        return deny(reply, "forbidden", 403);
      if (!config.enableLocalLaunch)
        return deny(reply, "local_launch_disabled", 403);
      ...
      if (!isLocalUrl(serverUrl) || !isLocalHost(requestHost))
        return deny(reply, "local_launch_requires_localhost", 400);

      const pairing = createAgentPairing(
        request.params.roomId,
        participant.id,
        body,
        serverUrl
      ...
      const launch = await localAgentLauncher({
        launchId,
        command: "corepack",
        args: pairingLaunchArgs(
          pairing.connection_code,
          connectorRuntimeDir,
          localRepoRoot
        ),
  ...
  app.post<{
    Params: { pairingToken: string };
    Body: {
      adapter_name?: string;
      working_dir?: string;
      compatibility?: unknown;
    };
    Querystring: { server_url?: string };
  }>("/agent-pairings/:pairingToken/claim", async (request, reply) => {
    if (!pairingClaimLimiter.allow(request.ip)) return tooMany(reply);
    const pairingHash = hashToken(
      request.params.pairingToken,
      config.tokenSecret
    );
    ...
    const claimResult = store.transaction(() => {
      ...
      if (pairing.claimed_at !== null)
        return { ok: false as const, error: "pairing_claimed", status: 409 };
      ...
      const agents = store
        ...
        .filter((p) => p.role === "agent");
      if (agents.length >= config.maxAgentsPerRoom)
        return { ok: false as const, error: "max_agents_reached", status: 409 };
      ...
      const added = store.addParticipant({
        ...
        id: agentId,
        token: agentToken,
        display_name: body.adapter_name ?? profile.name,
        type: "agent",
        role: "agent",
        main_thread_history_access: "allowed",
      });
      ...
      const events = [
        store.appendEvent(
          event(roomId, "agent.registered", pairing.created_by, {
            agent_id: agentId,
        ...
        store.appendEvent(
          event(roomId, "participant.joined", agentId, {
            participant: publicParticipant(added),
          })
        ),
        store.appendEvent(
          event(roomId, "agent.status_changed", agentId, {
            agent_id: agentId,
            status: "online",
          })
        ),
      ...
      if (shouldSelectAgent) {
        events.push(
          store.appendEvent(
            event(roomId, "room.agent_selected", pairing.created_by, {
              agent_id: agentId,
            })
          )
        );
    ...
    if (!claimResult.ok)
      return deny(reply, claimResult.error, claimResult.status);
    publishEvents(claimResult.events);
    return reply.code(201).send({
      room_id: claimResult.roomId,
      agent_id: agentId,
      agent_token: agentToken,
      agent: claimResult.profile,
      agent_type: claimResult.agentType,
      permission_level: claimResult.permissionLevel,
    });
  });

  app.post<{ Params: { roomId: string } }>(
    "/rooms/:roomId/agents/select",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (
        ...
        return deny(reply, "invalid_target_agent", 400);
      appendAndPublish(
        event(request.params.roomId, "room.agent_selected", participant.id, {
  ...
  app.post<{ Params: { roomId: string; agentId: string } }>(
    "/rooms/:roomId/agents/:agentId/thinking",
    async (request, reply) => {
      const actor = requireParticipant(store, request.params.roomId, request);
      if (!actor) return deny(reply, "invalid_token");
      if (!hasHumanRole(actor, ["owner"])) return deny(reply, "forbidden", 403);
      const body = z
        .object({ thinking_enabled: z.boolean() })
        .parse(request.body);
      const targetAgent = findParticipant(
        request.params.roomId,
      ...
      if (
        ...
        return deny(reply, "unknown_agent", 404);
      const updatedAt = new Date().toISOString();
      appendAndPublish(
        event(request.params.roomId, "agent.updated", actor.id, {
  ...
  app.post<{ Params: { roomId: string } }>(
    "/rooms/:roomId/claude/session-catalog",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (!assertAgentOwnsPayload(participant, body.agent_id))
        return deny(reply, "forbidden", 403);
      const validation = validateClaudeAgent(
        request.params.roomId,
        body.agent_id
      );
      if (!validation.ok)
        return deny(reply, validation.error, validation.status);
      appendAndPublish(
        event(
          request.params.roomId,
  ...
  app.post<{ Params: { roomId: string } }>(
    "/rooms/:roomId/claude/session-previews",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
  ...
  app.post<{ Params: { roomId: string; previewId: string } }>(
    "/rooms/:roomId/claude/session-previews/:previewId/messages",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (
        !body.every(
          (message) => message.preview_id === request.params.previewId
        )
      )
        return deny(reply, "preview_id_mismatch", 400);
      if (
        !body.every((message) =>
          assertAgentOwnsPayload(participant, message.agent_id)
        )
      )
        return deny(reply, "forbidden", 403);
      const first = body[0];
      ...
      if (!validation.ok)
        return deny(reply, validation.error, validation.status);
      const previewValidation = validatePreviewOpen(
        request.params.roomId,
      ...
      if (!previewValidation.ok)
        return deny(reply, previewValidation.error, previewValidation.status);
      const storedEvents = store.transaction(() =>
        body.map((message) =>
          store.appendEvent(
            event(
  ...
  app.post<{ Params: { roomId: string; previewId: string } }>(
    "/rooms/:roomId/claude/session-previews/:previewId/complete",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (body.preview_id !== request.params.previewId)
        return deny(reply, "preview_id_mismatch", 400);
      if (!assertAgentOwnsPayload(participant, body.agent_id))
        return deny(reply, "forbidden", 403);
      const validation = validateClaudeAgent(
      ...
      if (!previewValidation.ok)
        return deny(reply, previewValidation.error, previewValidation.status);
      const completeValidation = validatePreviewComplete(
        request.params.roomId,
        request.params.previewId,
        body
      );
      if (!completeValidation.ok)
        return deny(reply, completeValidation.error, completeValidation.status);
      appendAndPublish(
        event(
          request.params.roomId,
  ...
  app.post<{ Params: { roomId: string; previewId: string } }>(
    "/rooms/:roomId/claude/session-previews/:previewId/fail",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
  ...
  app.post<{ Params: { roomId: string } }>(
    "/rooms/:roomId/claude/session-selection",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
  ...
  app.post<{ Params: { roomId: string } }>(
    "/rooms/:roomId/claude/session-ready",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (!validation.ok)
        return deny(reply, validation.error, validation.status);
      const selectionValidation = validateSelectedClaudeSessionReady(
        request.params.roomId,
        body.agent_id,
        body
      );
  ...
  app.post<{ Params: { roomId: string } }>(
    "/rooms/:roomId/claude/session-imports/start",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (!validation.ok)
        return deny(reply, validation.error, validation.status);
      const selectionValidation = validateSelectedClaudeResumeSession(
        request.params.roomId,
        body.agent_id,
        body.session_id
      );
  ...
  app.post<{ Params: { roomId: string; importId: string } }>(
    "/rooms/:roomId/claude/session-imports/:importId/messages",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (
        !body.every((message) => message.import_id === request.params.importId)
      )
        return deny(reply, "import_id_mismatch", 400);
      if (
        !body.every((message) =>
          assertAgentOwnsPayload(participant, message.agent_id)
        )
        ...
        return deny(reply, "forbidden", 403);
      const first = body[0];
      const agentValidation = validateClaudeAgent(
        request.params.roomId,
        participant.id
      );
      if (!agentValidation.ok)
        return deny(reply, agentValidation.error, agentValidation.status);
      const selectionValidation = validateSelectedClaudeResumeSession(
        request.params.roomId,
        first.agent_id,
        first.session_id
      ...
      const batchValidation = validateImportMessageBatch(
        request.params.roomId,
        request.params.importId,
        body
      );
      if (!batchValidation.ok)
        return deny(reply, batchValidation.error, batchValidation.status);
      const storedEvents = store.transaction(() =>
        body.map((message) =>
          store.appendEvent(
            event(
  ...
  app.post<{ Params: { roomId: string; importId: string } }>(
    "/rooms/:roomId/claude/session-imports/:importId/complete",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (body.import_id !== request.params.importId)
        return deny(reply, "import_id_mismatch", 400);
      if (!assertAgentOwnsPayload(participant, body.agent_id))
        return deny(reply, "forbidden", 403);
      const validation = validateClaudeAgent(
      ...
      const completeValidation = validateImportComplete(
        request.params.roomId,
        request.params.importId,
        body
      );
  ...
  app.post<{ Params: { roomId: string; importId: string } }>(
    "/rooms/:roomId/claude/session-imports/:importId/fail",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
  ...
  app.post<{ Params: { roomId: string } }>(
    "/rooms/:roomId/agent-sessions/catalog",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (!assertAgentOwnsPayload(participant, body.agent_id))
        return deny(reply, "forbidden", 403);
      const validation = validateLocalAgentProvider(
        request.params.roomId,
        body.agent_id,
        body.provider
      );
  ...
  app.post<{ Params: { roomId: string } }>(
    "/rooms/:roomId/agent-sessions/selection",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
  ...
  app.post<{ Params: { roomId: string } }>(
    "/rooms/:roomId/agent-sessions/ready",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (!validation.ok)
        return deny(reply, validation.error, validation.status);
      const selectionValidation = validateSelectedLocalAgentSessionReady(
        request.params.roomId,
        body.agent_id,
        body.provider,
        body
  ...
  app.post<{ Params: { roomId: string } }>(
    "/rooms/:roomId/agent-sessions/previews",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
  ...
  app.post<{ Params: { roomId: string; previewId: string } }>(
    "/rooms/:roomId/agent-sessions/previews/:previewId/messages",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (
        !body.every(
          (message) =>
            message.agent_id === first.agent_id &&
      ...
      if (!validation.ok)
        return deny(reply, validation.error, validation.status);
      const previewValidation = validateAgentPreviewOpen(
        request.params.roomId,
        request.params.previewId,
        first.agent_id,
        first.provider,
  ...
  app.post<{ Params: { roomId: string; previewId: string } }>(
    "/rooms/:roomId/agent-sessions/previews/:previewId/complete",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (!previewValidation.ok)
        return deny(reply, previewValidation.error, previewValidation.status);
      const completeValidation = validateAgentPreviewComplete(
        request.params.roomId,
        request.params.previewId,
        body
      );
  ...
  app.post<{ Params: { roomId: string; previewId: string } }>(
    "/rooms/:roomId/agent-sessions/previews/:previewId/fail",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
  ...
  app.post<{ Params: { roomId: string } }>(
    "/rooms/:roomId/agent-sessions/imports/start",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (!validation.ok)
        return deny(reply, validation.error, validation.status);
      const selectionValidation = validateSelectedLocalAgentResumeSession(
        request.params.roomId,
        body.agent_id,
        body.provider,
        body.session_id
  ...
  app.post<{ Params: { roomId: string; importId: string } }>(
    "/rooms/:roomId/agent-sessions/imports/:importId/messages",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      const batchValidation = validateAgentImportMessageBatch(
        request.params.roomId,
        request.params.importId,
        body
      );
  ...
  app.post<{ Params: { roomId: string; importId: string } }>(
    "/rooms/:roomId/agent-sessions/imports/:importId/complete",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      const completeValidation = validateAgentImportComplete(
        request.params.roomId,
        request.params.importId,
        body
      );
  ...
  app.post<{ Params: { roomId: string; importId: string } }>(
    "/rooms/:roomId/agent-sessions/imports/:importId/fail",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
  ...
  app.post<{ Params: { roomId: string; runId: string } }>(
    "/rooms/:roomId/agent-runs/:runId/start",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (body.run_id !== request.params.runId)
        return deny(reply, "run_id_mismatch", 400);
      const validation = validateOpenAgentRun(
        request.params.roomId,
      ...
      if (!validation.ok)
        return deny(reply, validation.error, validation.status);
      const providerValidation = resolveValidatedAgentProvider(
        request.params.roomId,
        body.agent_id,
        body.provider
      );
      if (!providerValidation.ok)
        return deny(reply, providerValidation.error, providerValidation.status);
      const normalizedBody = { ...body, provider: providerValidation.provider };
      const runValidation = validateRunIsOpen(
        request.params.roomId,
        request.params.runId
      );
      if (!runValidation.ok)
        return deny(reply, runValidation.error, runValidation.status);
      if (runValidation.runState?.started_payload) {
        if (
          !hasMatchingPayload(
            runValidation.runState.started_payload,
            normalizedBody
          )
        )
          return deny(reply, "run_id_conflict", 409);
        return reply.code(201).send({ ok: true });
      }
      appendAndPublish(
        event(
  ...
  app.post<{ Params: { roomId: string; runId: string } }>(
    "/rooms/:roomId/agent-runs/:runId/complete",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (body.run_id !== request.params.runId)
        return deny(reply, "run_id_mismatch", 400);
      const validation = validateTerminalAgentRun(
        request.params.roomId,
        request.params.runId,
        body.turn_id,
        body.agent_id,
      ...
      if (!runValidation.ok)
        return deny(reply, runValidation.error, runValidation.status);
      appendAndPublish(
        event(request.params.roomId, "agent.run.completed", participant.id, {
          ...body,
          provider: providerValidation.provider,
        })
      );
      closePendingInteractionsForRun(
        request.params.roomId,
        request.params.runId,
        "run_closed"
      );
  ...
  app.post<{ Params: { roomId: string; runId: string } }>(
    "/rooms/:roomId/agent-runs/:runId/fail",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (!runValidation.ok)
        return deny(reply, runValidation.error, runValidation.status);
      appendAndPublish(
        event(request.params.roomId, "agent.run.failed", participant.id, {
          ...body,
          provider: providerValidation.provider,
        })
      );
  ...
  app.post<{ Params: { roomId: string; runId: string } }>(
    "/rooms/:roomId/agent-runs/:runId/nodes/start",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (!runValidation.ok)
        return deny(reply, runValidation.error, runValidation.status);
      const existingNode = findRunNodeState(
        request.params.roomId,
        request.params.runId,
        body.node_id
      );
      if (existingNode) {
        ...
        if (!nodeValidation.ok)
          return deny(reply, nodeValidation.error, nodeValidation.status);
        if (!hasMatchingPayload(existingNode.started_payload, normalizedBody))
  ...
  app.post<{ Params: { roomId: string; runId: string; nodeId: string } }>(
    "/rooms/:roomId/agent-runs/:runId/nodes/:nodeId/delta",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (body.run_id !== request.params.runId)
        return deny(reply, "run_id_mismatch", 400);
      if (body.node_id !== request.params.nodeId)
        return deny(reply, "node_id_mismatch", 400);
      const validation = validateOpenAgentRun(
        request.params.roomId,
        request.params.runId,
        body.turn_id,
      ...
      if (!runValidation.ok)
        return deny(reply, runValidation.error, runValidation.status);
      const nodeState = findRunNodeState(
        request.params.roomId,
        request.params.runId,
      ...
      if (!nodeValidation.ok)
        return deny(reply, nodeValidation.error, nodeValidation.status);
      appendAndPublish(
        event(request.params.roomId, "agent.run.node.delta", participant.id, {
          ...body,
          provider: providerValidation.provider,
        })
      );
  ...
  app.post<{ Params: { roomId: string; runId: string; nodeId: string } }>(
    "/rooms/:roomId/agent-runs/:runId/nodes/:nodeId/update",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (!nodeValidation.ok)
        return deny(reply, nodeValidation.error, nodeValidation.status);
      appendAndPublish(
        event(request.params.roomId, "agent.run.node.updated", participant.id, {
          ...body,
          provider: providerValidation.provider,
        })
      );
  ...
  app.post<{ Params: { roomId: string; runId: string; nodeId: string } }>(
    "/rooms/:roomId/agent-runs/:runId/nodes/:nodeId/complete",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      appendAndPublish(
        event(
          ...
          participant.id,
          { ...body, provider: providerValidation.provider }
        )
      );
      return reply.code(201).send({ ok: true });
    }
  ...
  app.post<{ Params: { roomId: string; runId: string; nodeId: string } }>(
    "/rooms/:roomId/agent-runs/:runId/nodes/:nodeId/fail",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (!nodeValidation.ok)
        return deny(reply, nodeValidation.error, nodeValidation.status);
      appendAndPublish(
        event(request.params.roomId, "agent.run.node.failed", participant.id, {
          ...body,
          provider: providerValidation.provider,
        })
      );
  ...
  app.post<{ Params: { roomId: string; runId: string; nodeId: string } }>(
    "/rooms/:roomId/agent-runs/:runId/approvals/:nodeId/request",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (!validation.ok)
        return deny(reply, validation.error, validation.status);
      const toolNode = findRunNodeState(
        request.params.roomId,
        request.params.runId,
        body.tool_node_id
      );
      if (!toolNode || toolNode.kind !== "tool")
        return deny(reply, "invalid_tool_node", 400);
      ...
      const key = pendingInteractionKey(
        request.params.roomId,
        request.params.runId,
        request.params.nodeId
      );
      const approvalRequestKey = buildRunNodeRequestKey(
        buildApprovalRequestShape(body)
      );
      const existingNode = findRunNodeState(
        request.params.roomId,
        request.params.runId,
      ...
      if (existing) {
        if (existing.requestKey !== approvalRequestKey)
          return deny(reply, "node_id_conflict", 409);
      ...
      if (terminalPayload) {
        const detail =
          typeof terminalPayload.detail === "object" &&
          terminalPayload.detail !== null
            ? (terminalPayload.detail as Record<string, unknown>)
            : {};
        return reply.code(201).send({
      ...
      const pending = createPendingApprovalEntry(
        request.params.roomId,
        request.params.runId,
        request.params.nodeId,
        body.agent_id,
        ...
        appendAndPublish(
          event(
            ...
            participant.id,
            {
              ...
              ...("source_refs" in approvalShape
                ? {
                    source_refs: approvalShape.source_refs as Record<
                      string,
                      unknown
                    >,
                  }
  ...
  app.post<{ Params: { roomId: string; runId: string; nodeId: string } }>(
    "/rooms/:roomId/agent-runs/:runId/approvals/:nodeId/resolve",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      return reply
        .code(201)
        .send({ ok: true, decision: body.decision, resolved_at: resolvedAt });
    }
  );

  app.post<{ Params: { roomId: string; runId: string; nodeId: string } }>(
    "/rooms/:roomId/agent-runs/:runId/elicitations/:nodeId/request",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      const elicitationRequestKey = buildRunNodeRequestKey(
        buildElicitationRequestShape(body, request.params.nodeId)
      );
      const existingNode = findRunNodeState(
        request.params.roomId,
        request.params.runId,
      ...
      if (
        existingNode &&
        !isMatchingElicitationNode(existingNode, body, request.params.nodeId)
      ) {
        return deny(reply, "node_id_conflict", 409);
      }
      const existing = pendingElicitations.get(key);
      if (existing) {
        if (existing.requestKey !== elicitationRequestKey)
          return deny(reply, "node_id_conflict", 409);
        const decision = await existing.promise;
        return reply.code(201).send(decision);
      }
      ...
      const pending = createPendingElicitationEntry(
        request.params.roomId,
        request.params.runId,
        request.params.nodeId,
        body.agent_id,
        provider,
        elicitationRequestKey
      );
      pending.timeout = setTimeout(() => {
        resolvePendingElicitation(key, {
          action: "cancel",
      ...
      if (!existingNode) {
        const elicitationShape = buildElicitationRequestShape(
          body,
          request.params.nodeId
        );
        appendAndPublish(
          event(
            ...
            participant.id,
            {
              ...
              source_refs: elicitationShape.source_refs as Record<
                string,
                unknown
              >,
              started_at: body.requested_at,
  ...
  app.post<{ Params: { roomId: string; runId: string; nodeId: string } }>(
    "/rooms/:roomId/agent-runs/:runId/elicitations/:nodeId/resolve",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      return reply
        .code(201)
        .send({ ok: true, action: body.action, resolved_at: resolvedAt });
    }
  );

  app.post<{ Params: { roomId: string; turnId: string } }>(
    "/rooms/:roomId/agent-turns/:turnId/start",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (!participant) return deny(reply, "invalid_token");
      const turn = requireAssignedAgentTurn(
        request.params.roomId,
      ...
      appendAndPublish(
        event(request.params.roomId, "agent.turn.started", participant.id, {
  ...
  app.post<{ Params: { roomId: string; turnId: string } }>(
    "/rooms/:roomId/agent-turns/:turnId/delta",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      appendAndPublish(
        event(request.params.roomId, "agent.output.delta", participant.id, {
          turn_id: request.params.turnId,
          agent_id: participant.id,
          chunk: body.chunk,
        })
  ...
  app.post<{ Params: { roomId: string; turnId: string } }>(
    "/rooms/:roomId/agent-turns/:turnId/complete",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      const storedEvents = store.transaction(() => {
        const completed = store.appendEvent(
          event(request.params.roomId, "agent.turn.completed", participant.id, {
            turn_id: request.params.turnId,
            agent_id: participant.id,
            message_id: messageId,
        ...
        const finalMessage = store.appendEvent(
          event(request.params.roomId, "message.created", participant.id, {
            message_id: messageId,
            content: { text: body.final_text, attachments: [] },
            kind: "agent",
            turn_id: request.params.turnId,
            created_at: new Date().toISOString(),
          })
        );
        const followupEvents = hasQueuedFollowup(
          store.listEvents(request.params.roomId),
          request.params.turnId
        )
          ? createAgentTurnRequestEvents(
              request.params.roomId,
  ...
  app.post<{ Params: { roomId: string; turnId: string } }>(
    "/rooms/:roomId/agent-turns/:turnId/fail",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      const storedEvents = store.transaction(() => {
        const failed = store.appendEvent(
          event(request.params.roomId, "agent.turn.failed", participant.id, {
            turn_id: request.params.turnId,
            agent_id: participant.id,
            ...failurePayload,
          })
  ...
  app.post<{ Params: { roomId: string } }>(
    ...
    async (request, reply) => {
      ...
      if (
        ...
        return deny(reply, "invalid_target_agent", 400);
      const capabilities = findAgentCapabilities(
        store.listEvents(request.params.roomId),
        body.target_agent_id
      );
      if (!capabilities.includes("legacy.task_runner"))
        return deny(reply, "generic_tasks_removed", 410);
      const taskId = prefixedId("task");
      appendAndPublish(
        event(request.params.roomId, "task.created", participant.id, {
          task_id: taskId,
          created_by: participant.id,
          ...body,
        })
  ...
  app.post<{ Params: { roomId: string; taskId: string } }>(
    "/rooms/:roomId/tasks/:taskId/start",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (!participant) return deny(reply, "invalid_token");
      const task = requireAssignedAgentTask(
        request.params.roomId,
      ...
      appendAndPublish(
        event(request.params.roomId, "task.started", participant.id, {
  ...
  app.post<{ Params: { roomId: string; taskId: string } }>(
    "/rooms/:roomId/tasks/:taskId/output",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      appendAndPublish(
        event(request.params.roomId, "task.output", participant.id, {
          task_id: request.params.taskId,
          agent_id: participant.id,
          ...TaskOutputSchema.parse(request.body),
        })
      );
      return reply.code(201).send({ ok: true });
    }
  ...
  app.post<{ Params: { roomId: string; taskId: string } }>(
    "/rooms/:roomId/tasks/:taskId/complete",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (!task.started) return deny(reply, "task_not_started", 409);
      appendAndPublish(
        event(request.params.roomId, "task.completed", participant.id, {
          task_id: request.params.taskId,
          agent_id: participant.id,
          ...TaskCompleteSchema.parse(request.body),
        })
      );
      return reply.code(201).send({ ok: true });
    }
  ...
  app.post<{ Params: { roomId: string; taskId: string } }>(
    "/rooms/:roomId/tasks/:taskId/fail",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (!task.started) return deny(reply, "task_not_started", 409);
      appendAndPublish(
        event(request.params.roomId, "task.failed", participant.id, {
          task_id: request.params.taskId,
          agent_id: participant.id,
          ...TaskFailedSchema.parse(request.body),
        })
      );
      return reply.code(201).send({ ok: true });
    }
  ...
  const ConnectorSnapshotRequestSchema = z.object({
    since_sequence: z.number().int().nonnegative().default(0),
  });
  const snapshotRequests = new Map<
    string,
    { requesterId: string; connectorId: string; sinceSequence: number }
  >();

  app.post<{ Params: { roomId: string } }>(
    "/rooms/:roomId/connector-snapshots",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (!participant) return deny(reply, "invalid_token");
      const canRequest =
        hasAnyRole(participant, ["owner", "admin"]) ||
        (participant.role === "member" &&
          participant.main_thread_history_access === "allowed");
      if (!canRequest) return deny(reply, "forbidden", 403);
      const activeAgentId = findActiveAgentId(
        store.listEvents(request.params.roomId)
      );
  ...
  app.post<{ Params: { roomId: string; requestId: string } }>(
    "/rooms/:roomId/connector-snapshots/:requestId/start",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (!participant) return deny(reply, "invalid_token");
      const snapshot = snapshotRequests.get(
        `${request.params.roomId}:${request.params.requestId}`
      );
      if (!snapshot) return deny(reply, "snapshot_not_found", 404);
      if (participant.id !== snapshot.connectorId)
        return deny(reply, "forbidden", 403);
      const body = z
        .object({
  ...
  app.post<{ Params: { roomId: string; requestId: string } }>(
    "/rooms/:roomId/connector-snapshots/:requestId/entries",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (participant.id !== snapshot.connectorId)
        return deny(reply, "forbidden", 403);
      const body = z
        .object({ entry: ConnectorLedgerEntrySchema })
        .parse(request.body);
      publishTargeted(
        event(
          request.params.roomId,
  ...
  app.post<{ Params: { roomId: string; requestId: string } }>(
    "/rooms/:roomId/connector-snapshots/:requestId/complete",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
      ...
      if (participant.id !== snapshot.connectorId)
        return deny(reply, "forbidden", 403);
      const body = z
        .object({ last_sequence: z.number().int().nonnegative() })
        .parse(request.body);
  ...
  app.post<{ Params: { roomId: string; requestId: string } }>(
    "/rooms/:roomId/connector-snapshots/:requestId/fail",
    async (request, reply) => {
      const participant = requireParticipant(
        store,
        request.params.roomId,
