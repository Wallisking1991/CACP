...
describe("cloud server endpoints", () => {
  it("creates durable invite links that work after restart", async () => {
    ...
    try {
      ...
      const inviteToken = inviteResponse.json<{ invite_token: string }>()
        .invite_token;
      await first.close();
      first = undefined;
  ...
  it("does not expose plaintext invite tokens in events", async () => {
    ...
    expect(JSON.stringify(eventsResponse.json().events)).not.toContain(
      inviteToken
    );
  ...
  it("disables start-local in cloud mode", async () => {
    ...
    const created = roomResponse.json<{
      room_id: string;
      owner_token: string;
    }>();
  ...
  it("rejects messages longer than configured max", async () => {
    ...
    const roomResponse = await app.inject({
      method: "POST",
      url: "/rooms",
      payload: { name: "Limit Room", display_name: "Alice" },
    });
    const created = roomResponse.json<{
      room_id: string;
      owner_token: string;
  ...
  it("rate limits room creation", async () => {
    ...
    const first = await app.inject({
      method: "POST",
      url: "/rooms",
      payload: { name: "Room A", display_name: "Alice" },
    });
    ...
    const second = await app.inject({
      method: "POST",
      url: "/rooms",
      payload: { name: "Room B", display_name: "Bob" },
    });
  ...
  it("enforces max participants per room at invite creation and approval", async () => {
    ...
    const roomResponse = await app.inject({
      method: "POST",
      url: "/rooms",
      payload: { name: "Small Room", display_name: "Alice" },
    });
    const created = roomResponse.json<{
      room_id: string;
      owner_token: string;
    ...
    expect(fullRoomInvite.json()).toMatchObject({
  ...
  it("enforces max agents per room", async () => {
    ...
    const roomResponse = await app.inject({
      method: "POST",
      url: "/rooms",
      payload: { name: "Agent Room", display_name: "Alice" },
    });
    const created = roomResponse.json<{
      room_id: string;
      owner_token: string;
  ...
  it("uses connector claim working directory when building the agent profile", async () => {
    ...
    expect(pairingResponse.statusCode).toBe(201);
    const pairingToken = parseConnectionCode(
      (pairingResponse.json() as { connection_code: string }).connection_code
    ).pairing_token;
    ...
    const claimResponse = await app.inject({
      ...
      payload: {
        compatibility: testConnectorCompatibility,
        adapter_name: "Local Echo",
        working_dir: localWorkingDir,
      },
    });
  ...
  it("falls back to pairing working_dir when claim omits working_dir", async () => {
    ...
    const pairingResponse = await app.inject({
      ...
      payload: {
        agent_type: "claude-code",
        permission_level: "read_only",
        working_dir: "D:\\Projects\\fallback",
      },
    });
    expect(pairingResponse.statusCode).toBe(201);
    const pairingToken = parseConnectionCode(
  ...
  it("verifies invite status without authentication", async () => {
    ...
    const roomResponse = await app.inject({
      method: "POST",
      url: "/rooms",
      payload: { name: "Verify Room", display_name: "Alice" },
    });
    const created = roomResponse.json<{
      room_id: string;
      owner_token: string;
    ...
    // Not found
    const notFoundCheck = await app.inject({
      method: "GET",
      url: "/invites/verify?token=invalid_token_xyz",
    });
  ...
  it("reports expired, revoked, and limit_reached invite states", async () => {
    ...
    const roomResponse = await app.inject({
      method: "POST",
      url: "/rooms",
      payload: { name: "States Room", display_name: "Alice" },
    });
    const created = roomResponse.json<{
      room_id: string;
      owner_token: string;
    ...
    const expiredToken = expiredInviteResponse.json<{ invite_token: string }>()
      .invite_token;
    ...
    const expiredCheck = await app.inject({
      method: "GET",
      url: `/invites/verify?token=${encodeURIComponent(expiredToken)}`,
    });
    ...
    const limitedToken = limitedInviteResponse.json<{ invite_token: string }>()
      .invite_token;

    const pending = await app.inject({
      method: "POST",
      url: `/rooms/${created.room_id}/join-requests`,
      payload: { invite_token: limitedToken, display_name: "Bob" },
    });
    expect(pending.statusCode).toBe(201);
    ...
    const revokedCheck = await app.inject({
      method: "GET",
      url: `/invites/verify?token=${encodeURIComponent(limitedToken)}`,
    });
    ...
    const freshToken = freshInviteResponse.json<{ invite_token: string }>()
      .invite_token;

    const freshPending = await app.inject({
      method: "POST",
      url: `/rooms/${created.room_id}/join-requests`,
      payload: { invite_token: freshToken, display_name: "Charlie" },
    });
    ...
    const limitCheck = await app.inject({
      method: "GET",
      url: `/invites/verify?token=${encodeURIComponent(freshToken)}`,
    });
    expect(limitCheck.statusCode).toBe(200);
    expect(limitCheck.json()).toEqual({
      valid: false,
