...
import {
  deriveMainInputQueue,
  nextQueuedMainInput,
} from "../src/main-inputs.js";
...
describe("deriveMainInputQueue", () => {
  ...
  it("returns queued items in FIFO order", () => {
    const events = [
      ...
      makeEvent("main_input.accepted", {
        input_id: "i2",
        author_id: "u2",
        content: { text: "World", attachments: [] },
        source: "composer",
        created_at: "2026-05-01T00:00:01.000Z",
      }),
      makeEvent("main_input.queued", {
        input_id: "i2",
        queued_after_turn_id: "t2",
      }),
    ];
  ...
  it("excludes cancelled inputs", () => {
    const events = [
      ...
      makeEvent("main_input.cancelled", {
        input_id: "i1",
        cancelled_by: "u_owner",
      }),
    ];
  ...
  it("excludes triggered inputs", () => {
    const events = [
      ...
      makeEvent("main_input.triggered", {
        input_id: "i1",
        trigger_turn_id: "t2",
      }),
    ];
  ...
  it("excludes failed inputs", () => {
    const events = [
      ...
      makeEvent("main_input.failed", {
        input_id: "i1",
        failure_reason: "agent_offline",
      }),
    ];
    expect(deriveMainInputQueue(events)).toEqual([]);
  });
  ...
  it("returns orbit_promote source correctly", () => {
    const events = [
      makeEvent("main_input.accepted", {
        input_id: "i1",
        author_id: "u1",
        content: { text: "Promoted", attachments: [] },
  ...
  it("returns the first queued item", () => {
    const events = [
      makeEvent("main_input.accepted", {
        input_id: "i1",
        author_id: "u1",
        content: { text: "First", attachments: [] },
        source: "composer",
        created_at: "2026-05-01T00:00:00.000Z",
      }),
      makeEvent("main_input.queued", {
      ...
      makeEvent("main_input.accepted", {
        input_id: "i2",
        author_id: "u2",
        content: { text: "Second", attachments: [] },
        source: "composer",
        created_at: "2026-05-01T00:00:01.000Z",
      }),
      makeEvent("main_input.queued", {
