...
describe("participant role update", () => {
  ...
  it("lets owner update a member role to admin", async () => {
    ...
    const roleEvent = events.find(
      (e: { type: string }) => e.type === "participant.role_updated"
    );
  ...
  it("lets admin approve join requests after role change", async () => {
    ...
    const alice = await joinViaApproval(
      app,
      room.room_id,
      room.owner_token,
      memberInvite.invite_token,
      "Alice"
    );
    ...
    const pending = await app.inject({
      ...
      payload: { invite_token: newInvite.invite_token, display_name: "Bob" },
    });
    expect(pending.statusCode).toBe(201);
