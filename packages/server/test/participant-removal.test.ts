...
describe("participant removal", () => {
  ...
  it("revokes a member token and records a removal event", async () => {
    ...
    const joined = await joinViaApproval(
      app,
      room.room_id,
      room.owner_token,
      invite.invite_token,
    ...
    expect(
      events.events.some(
        (event) =>
          ...
          event.payload.participant_id === joined.participant_id
      )
    ).toBe(true);
  });
  ...
  it("owner leave revokes everyone and records room shutdown events", async () => {
    ...
    try {
      ...
      ).json() as { room_id: string; owner_token: string; owner_id: string };
  ...
  it("clears active agent selection when an agent is removed", async () => {
    ...
    expect(agentResponse.statusCode).toBe(201);
    const agentA = agentResponse.json() as {
      agent_id: string;
      agent_token: string;
    };
    ...
    const selectA = await app.inject({
      ...
      headers: ownerAuth,
      payload: { agent_id: agentA.agent_id },
    });
    ...
    const deselectionEvents = events.filter(
      (e) => e.type === "room.agent_selected" && e.payload.agent_id === ""
    );
    ...
    const agentB = agentBResponse.json() as {
      agent_id: string;
      agent_token: string;
    };
    ...
    const selectB = await app.inject({
      ...
      url: `/rooms/${room.room_id}/agents/select`,
      headers: ownerAuth,
      payload: { agent_id: agentB.agent_id },
    });
