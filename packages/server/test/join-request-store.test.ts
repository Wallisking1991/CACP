...
describe("join requests and participant revocations", () => {
  ...
  it("transitions a pending request once", () => {
    ...
    expect(() =>
      store.rejectJoinRequest(
        "join_alpha",
        "2026-04-27T08:03:00.000Z",
        "user_owner"
      )
    ).toThrow("join_request_not_pending");
    store.close();
  });
  ...
  it("marks participant tokens revoked", () => {
    ...
    expect(store.getParticipantByToken("room_alpha", "cacp_token")?.id).toBe(
      "user_alice"
    );
    ...
    expect(
      store.getParticipantByToken("room_alpha", "cacp_token")
    ).toBeUndefined();
