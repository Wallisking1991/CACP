...
async function setupClaudeSession(
  app: Awaited<ReturnType<typeof buildServer>>,
  roomId: string,
  ownerAuth: { authorization: string },
  agentId: string,
...
async function approveProposal(
  ...
  const response = await app.inject({
    method: "POST",
    url: `/rooms/${roomId}/proposals/${proposalId}/votes`,
    headers: ownerAuth,
...
describe("CACP server hardening", () => {
  ...
  it("rejects additional votes on a terminal proposal without appending duplicate events", async () => {
    ...
    const secondVote = await app.inject({
      method: "POST",
      url: `/rooms/${room.room_id}/proposals/${proposal.proposal_id}/votes`,
      headers: ownerAuth,
      payload: { vote: "reject" },
    });
    ...
    const proposalEvents = events.filter(
      (event) => event.payload.proposal_id === proposal.proposal_id
    );
    expect(
      proposalEvents.filter((event) => event.type === "proposal.vote_cast")
    ).toHaveLength(1);
    expect(
      proposalEvents.filter((event) => event.type === "proposal.approved")
    ).toHaveLength(1);
  });
  ...
  it("rejects proposal votes because the live room ends on restart", async () => {
    ...
    const secondVote = await second.inject({
      method: "POST",
      url: `/rooms/${room.room_id}/proposals/${proposal.proposal_id}/votes`,
      headers: ownerAuth,
      payload: { vote: "reject" },
  ...
  it("does not revive persisted invite or proposal state after restart", async () => {
    ...
    const invite = await first.inject({
      method: "POST",
    ...
    const proposal = await first.inject({
      method: "POST",
      url: `/rooms/${room.room_id}/proposals`,
      headers: ownerAuth,
      payload: {
  ...
  it("rejects voting on a proposal through another room route", async () => {
    ...
    const proposal = await app.inject({
      method: "POST",
      url: `/rooms/${roomA.room.room_id}/proposals`,
      headers: roomA.ownerAuth,
      payload: {
        title: "Room A only",
        proposal_type: "proposal",
        policy: { type: "owner_approval" },
    ...
    const wrongRoomVote = await app.inject({
      method: "POST",
      url: `/rooms/${roomB.room.room_id}/proposals/${proposal.json().proposal_id}/votes`,
      headers: roomB.ownerAuth,
      payload: { vote: "approve" },
    });
  ...
  it("allows only the assigned agent to emit task lifecycle events for existing tasks", async () => {
    const { app, room, ownerAuth } = await createRoom();
    const assigned = await registerAgent(
      app,
      room.room_id,
      ownerAuth,
      "Assigned Agent"
    ...
    for (const [action, payload] of [
      ...
      expect(
        (
          await app.inject({
            method: "POST",
            url: `/rooms/${room.room_id}/tasks/${task.task_id}/${action}`,
            headers: otherAuth,
            payload,
          })
        ).statusCode
      ...
      expect(
        (
          await app.inject({
            method: "POST",
            url: `/rooms/${room.room_id}/tasks/task_missing/${action}`,
            headers: assignedAuth,
            payload,
          })
        ).statusCode
      ).toBe(404);
    ...
    const failTask = await createTask(
      app,
      room.room_id,
      ownerAuth,
      assigned.agent_id
    ...
    expect(
      (
        await app.inject({
          method: "POST",
          url: `/rooms/${room.room_id}/tasks/${failTask.task_id}/start`,
          headers: assignedAuth,
          payload: {},
        })
      ).statusCode
    ...
    expect(
      (
        await app.inject({
          method: "POST",
          url: `/rooms/${room.room_id}/tasks/${failTask.task_id}/fail`,
          headers: assignedAuth,
          payload: { error: "expected failure", exit_code: 1 },
        })
      ).statusCode
    ).toBe(201);
  });
  ...
  it("rejects task creation for missing, human, observer, or cross-room target agents", async () => {
    ...
    const roomA = await createRoom(app, "A Owner");
    const roomB = await createRoom(app, "B Owner");
    const roomBAgent = await registerAgent(
      app,
      roomB.room.room_id,
      roomB.ownerAuth,
      "Room B Agent"
    ...
    const observerInvite = await app.inject({
      method: "POST",
      url: `/rooms/${roomA.room.room_id}/invites`,
      headers: roomA.ownerAuth,
      payload: { role: "observer" },
    });
    const observerJoin = await joinViaApproval(
      app,
      roomA.room.room_id,
      roomA.ownerAuth,
      observerInvite.json().invite_token,
      "Watcher"
    );
    ...
    const targetIds = [
      "agent_missing",
      roomA.room.owner_id,
      observerJoin.participant_id,
      roomBAgent.agent_id,
    ];
    for (const targetAgentId of targetIds) {
      const response = await app.inject({
        method: "POST",
        url: `/rooms/${roomA.room.room_id}/tasks`,
        headers: roomA.ownerAuth,
        payload: {
          target_agent_id: targetAgentId,
          prompt: "No",
  ...
  it("denies a token from one room when reading another room events", async () => {
    ...
    const response = await app.inject({
      method: "GET",
      url: `/rooms/${roomB.room.room_id}/events`,
      headers: roomA.ownerAuth,
    });
  ...
  it("restricts control APIs to owners/admins", async () => {
    ...
    const ownerAgent = await registerAgent(
      app,
      room.room_id,
      ownerAuth,
      "Owner Agent"
    ...
    const memberAttempts = [
      ...
      app.inject({
        ...
        headers: memberAuth,
        payload: { agent_id: ownerAgent.agent_id },
      }),
    ];
    expect(
      (await Promise.all(memberAttempts)).map((response) => response.statusCode)
    ).toEqual([403, 403, 403]);

    updateParticipantRole(dbPath, room.room_id, join.participant_id, "admin");
    const adminAuth = memberAuth;
    const adminAgent = await app.inject({
      method: "POST",
      url: `/rooms/${room.room_id}/agents/register`,
      headers: adminAuth,
      payload: {
    ...
    expect(
      (
        await app.inject({
          ...
          url: `/rooms/${room.room_id}/agents/select`,
          headers: adminAuth,
          payload: { agent_id: adminAgent.json().agent_id },
        })
      ).statusCode
    ).toBe(201);
    expect(
  ...
  it("restricts room active agent selection to human collaborators and same-room agents", async () => {
    ...
    const roomA = await createRoom(app, "A Owner");
    const roomB = await createRoom(app, "B Owner");
    const roomAAgent = await registerAgent(
      app,
      roomA.room.room_id,
      roomA.ownerAuth,
      "Room A Agent"
    ...
    expect(
      (
        await app.inject({
          method: "POST",
          url: `/rooms/${roomA.room.room_id}/agents/select`,
          headers: roomA.ownerAuth,
          payload: { agent_id: roomAAgent.agent_id },
        })
      ).statusCode
    ).toBe(201);
    expect(
      (
        await app.inject({
          ...
          url: `/rooms/${roomA.room.room_id}/agents/select`,
          headers: {
            authorization: `Bearer ${observerJoin.participant_token}`,
          },
          payload: { agent_id: roomAAgent.agent_id },
        })
      ).statusCode
    ...
    expect(
      (
        await app.inject({
          method: "POST",
          url: `/rooms/${roomA.room.room_id}/agents/select`,
          headers: { authorization: `Bearer ${roomAAgent.agent_token}` },
          payload: { agent_id: roomAAgent.agent_id },
        })
      ).statusCode
    ).toBe(403);
    expect(
      (
        await app.inject({
          ...
          url: `/rooms/${roomA.room.room_id}/agents/select`,
          headers: roomA.ownerAuth,
          payload: { agent_id: roomBAgent.agent_id },
        })
      ).statusCode
    ).toBe(400);
    expect(
  ...
  it("allows only the assigned agent to emit turn lifecycle events and enforces ordering", async () => {
    ...
    expect(
      (
        await app.inject({
          ...
          url: `/rooms/${room.room_id}/agents/select`,
          headers: ownerAuth,
          payload: { agent_id: assigned.agent_id },
        })
      ).statusCode
    ).toBe(201);
    ...
    expect(
      (
        await app.inject({
          method: "POST",
          url: `/rooms/${room.room_id}/agent-turns/${turnId}/delta`,
          headers: assignedAuth,
          payload: { chunk: "too early" },
        })
      ).statusCode
    ).toBe(409);
    expect(
      (
        await app.inject({
          ...
          headers: assignedAuth,
          payload: { final_text: "too early", exit_code: 0 },
        })
      ).statusCode
    ).toBe(409);
  ...
  it("enforces task lifecycle ordering and terminal state", async () => {
    ...
    const beforeStartTask = await createTask(
      app,
      room.room_id,
      ownerAuth,
      agent.agent_id
    );
    const outputBeforeStart = await app.inject({
      method: "POST",
      url: `/rooms/${room.room_id}/tasks/${beforeStartTask.task_id}/output`,
      headers: agentAuth,
      payload: { stream: "stdout", chunk: "too early" },
    });
    expect(outputBeforeStart.statusCode).toBe(409);
    expect(outputBeforeStart.json()).toEqual({ error: "task_not_started" });
    const completeBeforeStart = await app.inject({
      method: "POST",
      url: `/rooms/${room.room_id}/tasks/${beforeStartTask.task_id}/complete`,
      headers: agentAuth,
      payload: { exit_code: 0 },
    });
    ...
    expect(
      (
        await app.inject({
          method: "POST",
          url: `/rooms/${room.room_id}/tasks/${task.task_id}/output`,
          headers: agentAuth,
          payload: { stream: "stdout", chunk: "ok" },
        })
      ).statusCode
    ).toBe(201);
    expect(
