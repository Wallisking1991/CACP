...
describe("CACP server hardening", () => {
  it("does not share invites or proposal state across buildServer instances", async () => {
    ...
    const proposal = await first.inject({ method: "POST", url: `/rooms/${room.room_id}/proposals`, headers: ownerAuth, payload: { title: "Instance local", proposal_type: "decision", policy: { type: "owner_approval" } } });
    ...
    expect((await second.inject({ method: "POST", url: `/rooms/${room.room_id}/proposals/${proposal.json().proposal_id}/votes`, headers: ownerAuth, payload: { vote: "approve" } })).statusCode).toBe(404);
  });
  ...
  it("allows only the assigned agent to emit task lifecycle events for existing tasks", async () => {
    ...
    const task = await app.inject({ method: "POST", url: `/rooms/${room.room_id}/tasks`, headers: ownerAuth, payload: { target_agent_id: assigned.agent_id, prompt: "Do work", mode: "oneshot" } });
    ...
    for (const [action, payload] of [
      ...
      expect((await app.inject({ method: "POST", url: `/rooms/${room.room_id}/tasks/${taskId}/${action}`, headers: otherAuth, payload })).statusCode).toBe(403);
  ...
  it("denies a token from one room when reading another room events", async () => {
    ...
    const server = app ?? await trackedServer();
  const response = await server.inject({ method: "GET", url: `/rooms/${roomB.room.room_id}/events`, headers: roomA.ownerAuth });
