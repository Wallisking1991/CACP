...
describe("invite-derived history access", () => {
  ...
  it("defaults member invite to allowed and observer to denied", async () => {
    ...
    expect(memberInvite.json()).toMatchObject({
    ...
    expect(observerInvite.json()).toMatchObject({
      main_thread_history_access: "denied",
    });
  });
  ...
  it("accepts explicit main_thread_history_access on invite creation", async () => {
    ...
    expect(deniedMember.json()).toMatchObject({
      main_thread_history_access: "denied",
    });
    ...
    expect(allowedObserver.json()).toMatchObject({
      main_thread_history_access: "allowed",
    });
  });
  ...
  it("returns main_thread_history_access from /rooms/:roomId/me", async () => {
    ...
    const { participant_token } = status.json() as {
