...
function splitVisible(
  avatars: AvatarStatusView[],
  maxVisible: number
): { visible: AvatarStatusView[]; hiddenCount: number } {
...
export function RoleAvatarRail({
  avatars,
  maxVisible = 10,
  isOwner,
  ...
  const { visible, hiddenCount } = splitVisible(
    [...agents, ...humans],
    maxVisible
  );
  ...
  return (
    <div
      className="role-avatar-rail"
      ref={railRef}
      aria-label={t("room.controls")}
    >
      {visible
        .filter((avatar) => avatar.group === "agents")
        .map((avatar) => {
          ...
          return (
            <div
              ...
              data-agent-active={
                avatar.status === "working" || avatar.status === "typing"
                  ? "true"
                  : undefined
              }
            ...
            >
              ...
              >
                <span className="role-avatar__initials">
                  {initials(avatar.display_name)}
                </span>
                <span className="role-avatar__icon" aria-hidden="true">
                  🤖
                </span>
                <span className="role-avatar__halo" aria-hidden="true" />
              </span>
              <span className="role-avatar__name" style={{ color: colors.bar }}>
                {avatar.display_name}
              </span>
              {isOwner &&
                currentParticipantId !== avatar.id &&
                onRemoveAvatar && (
                  <button
                    type="button"
      ...
      {visible
        .filter((avatar) => avatar.group === "humans")
        .map((avatar) => {
      ...
      {hiddenCount > 0 ? (
        <span className="role-avatar-overflow">+{hiddenCount}</span>
      ) : null}
    </div>
  );
}
