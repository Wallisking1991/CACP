  ...
  return (
    <div className="modal-overlay" role="presentation">
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-label={t("joinRequestModal.title")}
      >
        <p className="landing-eyebrow" style={{ marginBottom: 8 }}>
          {t("joinRequestModal.title")}
        </p>
        <h3>{t("joinRequestModal.body", { name: request.display_name })}</h3>
        {remainingCount > 0 && (
          <p className="join-request-modal-subcopy">
            {t("joinRequestModal.more", { count: remainingCount })}
          </p>
        )}
        <div className="join-request-modal-actions">
          ...
          <button
            type="button"
            className="btn btn-ghost"
            onClick={() => onLater(request.request_id)}
          >
