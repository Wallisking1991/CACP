...
export default function ConnectionCodeModal({
  pairing,
  onClose,
  ...
  const handleCopy = useCallback(() => {
    ...
    navigator.clipboard
      .writeText(pairing.connection_code)
      .then(() => {
        setCopied(true);
  ...
  return (
    <div className="modal-overlay" role="presentation">
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-label={t("connectorModal.title")}
      >
        ...
        <p style={{ color: "var(--ink-4)", fontSize: 12, margin: "12px 0 0" }}>
          {t("connectorModal.expires", {
            expiresAt: new Date(pairing.expires_at).toLocaleString(),
          })}
        </p>
        {copyFailed && (
          <div style={{ marginTop: 12 }}>
            <p className="error inline-error" style={{ marginBottom: 8 }}>
              {t("connectorModal.copyFailed")}
            </p>
            <textarea
              className="input"
              readOnly
