...
describe("isTurnInFlight", () => {
  ...
  it("returns false after agent.turn.completed", () => {
    const events = [
      ...
      event(
        ...
        { turn_id: "t1", agent_id: "agent_1", message_id: "msg_1" },
        2,
        "agent_1"
      ),
    ];
  ...
  it("returns false after agent.turn.failed", () => {
    const events = [
      ...
      event(
        "agent.turn.failed",
        { turn_id: "t1", agent_id: "agent_1", error: "boom" },
        2,
        "agent_1"
      ),
    ];
...
describe("humanParticipants", () => {
  it("excludes agent participants from people counts and lists", () => {
    ...
    expect(
      humanParticipants(participants).map((participant) => participant.id)
    ).toEqual(["user_owner", "user_member", "user_observer"]);
  });
});
