...
describe("room state agent status in message cards", () => {
    ...
    const state = deriveRoomState([
      ...
      event(
        "claude.runtime.status_changed",
        {
          ...
          started_at: "2026-04-25T00:00:01.000Z",
          updated_at: "2026-04-25T00:00:04.000Z",
        },
        3,
        "agent_1"
      ),
  ...
  it("marks turn failed on same message card instead of creating system message", () => {
    const state = deriveRoomState([
      ...
      event(
        "agent.output.delta",
        { turn_id: "turn_1", agent_id: "agent_1", chunk: "Partial output" },
        2,
        "agent_1"
      ),
      event(
  ...
  it("treats thinking deltas as private status without exposing raw text", () => {
    const state = deriveRoomState([
      ...
      event(
        ...
        { agent_id: "agent_1", turn_id: "turn_1", text: "", done: false },
        2,
        "agent_1"
      ),
      event(
      ...
      event(
        "claude.output.thinking_delta",
        { agent_id: "agent_1", turn_id: "turn_1", text: "", done: true },
        5,
        "agent_1"
      ),
    ]);
  ...
  it("attaches final status from agent.runtime.status_completed (Codex provider)", () => {
    const state = deriveRoomState([
      ...
      event(
        "agent.output.delta",
        { turn_id: "turn_1", agent_id: "agent_1", chunk: "OK" },
        2,
        "agent_1"
      ),
      event(
