...
describe("ClaudeSessionPicker", () => {
  it("requests and shows complete preview content before selecting resume", async () => {
    ...
    fireEvent.click(
      screen.getAllByRole("button", { name: /Inspect latest: Planning/ })[0]
    );
    expect(
    ...
    expect(
      screen.getByText(/upload the complete selected Claude Code session/)
    ).toBeInTheDocument();
  ...
  it("opens inspected history in a modal outside the scrollable session list", async () => {
    ...
    expect(
      container.querySelector(".claude-session-list")?.contains(dialog)
    ).toBe(false);
    expect(
      screen.getByRole("heading", { name: "History 17" })
    ).toBeInTheDocument();
    await waitFor(() =>
      expect(onRequestPreview).toHaveBeenCalledWith("session_17")
    );
  ...
  it("keeps wantsReselect=true when a parent re-renders with a fresh-but-equivalent selection object", () => {
    ...
    expect(
      screen.queryByRole("button", { name: /Change selection/i })
