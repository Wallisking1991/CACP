...
import { deriveRoomState, type RoomStateEvent } from "../src/room-state.js";
...
describe("room state", () => {
  it("derives participants, agents, active agent, messages, and streaming turns", () => {
    const state = deriveRoomState([
      ...
      event(
        "agent.output.delta",
        { turn_id: "turn_1", agent_id: "agent_1", chunk: "work" },
        6,
        "agent_1"
      ),
      event(
        "agent.output.delta",
        { turn_id: "turn_1", agent_id: "agent_1", chunk: "ing" },
        7,
        "agent_1"
      ),
    ]);
    ...
    expect(state.messages).toEqual([
      {
        ...
        text: "hello",
        kind: "human",
        created_at: "2026-04-25T00:00:04.000Z",
      },
    ]);
    expect(state.streamingTurns).toEqual([
      { turn_id: "turn_1", agent_id: "agent_1", text: "working" },
    ]);
  });
  ...
  it("removes streaming turns after completion", () => {
    const state = deriveRoomState([
      ...
      event(
        "agent.output.delta",
        { turn_id: "turn_1", agent_id: "agent_1", chunk: "done" },
        2,
        "agent_1"
      ),
      event(
    ...
    const state = deriveRoomState([
      ...
      event(
        "agent.output.delta",
        {
          turn_id: "turn_1",
          agent_id: "agent_1",
          chunk: "error: invalid permission mode\n",
        },
        2,
        "agent_1"
      ),
  ...
  it("derives agent online status", () => {
    const state = deriveRoomState([
      event(
        "agent.registered",
        { agent_id: "agent_1", name: "Claude", capabilities: ["repo.read"] },
        1
      ),
      event(
        "agent.status_changed",
        { agent_id: "agent_1", status: "online" },
        2,
        "agent_1"
      ),
    ]);
  ...
  it("removes agent from agents map when participant.removed is followed by agent.status_changed", () => {
    const state = deriveRoomState([
      ...
      event(
        "agent.status_changed",
        { agent_id: "agent_1", status: "offline" },
        4,
        "agent_1"
      ),
      event(
        "participant.removed",
        {
          ...
          removed_by: "agent_1",
          removed_at: "2026-04-25T00:00:05.000Z",
          reason: "disconnected",
        },
        5,
        "agent_1"
  ...
  it("derives generic Codex session catalog and selection state", () => {
    const state = deriveRoomState([
      event(
        "agent.registered",
        { agent_id: "agent_1", name: "Codex", capabilities: ["codex-cli"] },
        1,
        "owner"
      ),
      event(
  ...
  it("derives generic Codex session readiness separately from selection", () => {
    const state = deriveRoomState([
      ...
      event(
        "agent.session_ready" as CacpEvent["type"],
        {
          agent_id: "agent_1",
          provider: "codex-cli",
          mode: "fresh",
  ...
  it("derives Claude session catalog and selection state", () => {
    const state = deriveRoomState([
      ...
      event(
        "agent.registered",
        { agent_id: "agent_1", name: "Claude", capabilities: ["claude-code"] },
        2,
        "owner"
      ),
      event(
    ...
    expect(state.claudeSessionCatalog?.sessions[0].session_id).toBe(
      "session_1"
    );
  ...
  it("derives owner-only Claude session preview content outside the main timeline", () => {
    const state = deriveRoomState([
      event(
        "claude.session_preview.requested" as CacpEvent["type"],
        {
      ...
      event(
        "claude.session_preview.message" as CacpEvent["type"],
        {
          preview_id: "preview_1",
          agent_id: "agent_1",
          session_id: "session_1",
      ...
      event(
        "claude.session_preview.completed" as CacpEvent["type"],
        {
          preview_id: "preview_1",
          agent_id: "agent_1",
          session_id: "session_1",
    ...
    expect(state.messages).toEqual([]);
    expect(state.claudeSessionPreviews).toEqual([
      expect.objectContaining({
        preview_id: "preview_1",
        session_id: "session_1",
        status: "completed",
        messages: [
          expect.objectContaining({ text: "Preview-only user message" }),
        ],
      }),
    ]);
  });
  ...
  it("derives owner-only generic Codex session preview content outside the main timeline", () => {
    const state = deriveRoomState([
      event(
        "agent.session_preview.requested" as CacpEvent["type"],
        {
          preview_id: "preview_1",
          agent_id: "agent_1",
          provider: "codex-cli",
      ...
      event(
        "agent.session_preview.message" as CacpEvent["type"],
        {
          preview_id: "preview_1",
          agent_id: "agent_1",
          provider: "codex-cli",
      ...
      event(
        "agent.session_preview.completed" as CacpEvent["type"],
        {
          preview_id: "preview_1",
          agent_id: "agent_1",
          provider: "codex-cli",
    ...
    expect(state.messages).toEqual([]);
    expect((state as any).agentSessionPreviews).toEqual([
      expect.objectContaining({
        preview_id: "preview_1",
        provider: "codex-cli",
        session_id: "session_1",
  ...
  it("renders completed Claude imports in the main message timeline", () => {
    const state = deriveRoomState([
      ...
      event(
        ...
        {
          message_id: "msg_1",
          content: { text: "Continue below", attachments: [] },
          kind: "human",
        },
        5,
        "owner"
  ...
  it("renders completed generic Codex imports in the main message timeline", () => {
    const state = deriveRoomState([
      event(
        "agent.session_import.started" as CacpEvent["type"],
        {
          import_id: "import_1",
          agent_id: "agent_1",
          provider: "codex-cli",
      ...
      event(
        "agent.session_import.message" as CacpEvent["type"],
        {
          import_id: "import_1",
          agent_id: "agent_1",
          provider: "codex-cli",
      ...
      event(
        "agent.session_import.completed" as CacpEvent["type"],
        {
          import_id: "import_1",
          agent_id: "agent_1",
          provider: "codex-cli",
    ...
    expect((state as any).agentImports).toEqual([
      expect.objectContaining({
        import_id: "import_1",
        provider: "codex-cli",
        status: "completed",
  ...
  it("keeps imported Claude transcript contiguous before room messages posted during import", () => {
    const state = deriveRoomState([
      ...
      event(
        "message.created",
        {
          message_id: "msg_during_import",
          content: {
            text: "Human typed while import was running",
            attachments: [],
          },
          kind: "human",
        },
  ...
  it("shows a failed Claude import even when failure happens before import start", () => {
    ...
    expect(state.claudeImports).toEqual([
      expect.objectContaining({
        import_id: "import_1",
        agent_id: "agent_1",
        session_id: "session_1",
    ...
    const state = deriveRoomState([
      ...
      event(
        "claude.runtime.status_changed",
        {
          agent_id: "agent_1",
          turn_id: "turn_2",
          status_id: "status_turn_2",
          phase: "thinking",
          current: "T2",
          recent: ["T2"],
          metrics: { files_read: 0, searches: 0, commands: 0 },
          started_at: "2026-04-29T00:00:03.000Z",
          updated_at: "2026-04-29T00:00:04.000Z",
        },
        3,
        "agent_1"
      ),
  ...
  it("derives participant activity and avatar statuses with priority", () => {
      ...
      [
        ...
        event(
          "agent.registered",
          {
            agent_id: "agent_1",
            name: "Claude Code Agent",
            capabilities: ["claude-code", "repo.read"],
          },
          3,
          "agent_1"
        ),
        ...
        event(
          "participant.presence_changed",
          {
            participant_id: "user_2",
            presence: "idle",
            updated_at: "2026-04-25T00:00:05.000Z",
          },
          5,
          "user_2"
        ),
        event(
          "participant.typing_started",
          {
            participant_id: "user_2",
            scope: "room",
            started_at: "2026-04-25T00:00:06.000Z",
          },
          6,
          "user_2"
        ),
    ...
    expect(state.participantActivity.get("user_2")).toMatchObject({
      presence: "idle",
      typing: true,
    });
    expect(
      state.avatarStatuses.find((item) => item.id === "user_2")
    ).toMatchObject({ kind: "human", status: "typing", group: "humans" });
    expect(
      state.avatarStatuses.find((item) => item.id === "agent_1")
    ).toMatchObject({ kind: "agent", status: "working", group: "agents" });
  });
      ...
      [
        ...
        event(
          "participant.typing_stopped",
          {
            participant_id: "user_2",
            scope: "room",
            stopped_at: "2026-04-25T00:00:02.000Z",
          },
          3,
          "user_2"
        ),
    ...
    const state = deriveRoomState([
      ...
      event(
        "join_request.created",
        {
          ...
          display_name: "Bob",
          role: "member",
          requested_at: "2026-04-26T00:01:00.000Z",
          expires_at: "2026-04-26T00:11:00.000Z",
        },
        4
      ),
      event(
      ...
      event(
        "join_request.created",
        {
          ...
          display_name: "Carol",
          role: "member",
          requested_at: "2026-04-26T00:02:00.000Z",
          expires_at: "2026-04-26T00:12:00.000Z",
        },
        6
      ),
      event(
      ...
      event(
        "invite.revoked",
        { invite_id: "inv_1", revoked_at: "2026-04-26T00:08:00.000Z" },
        8,
        "user_1"
      ),
    ]);
    ...
    it("clears orbit notes when orbit.cleared arrives", () => {
      const state = deriveRoomState(
        [
          ...
          event(
            "orbit.cleared",
            { cleared_by: "user_1", cleared_at: "2026-05-01T12:00:01.000Z" },
            2,
            "user_1"
          ),
        ],
  ...
  describe("Main input queue events", () => {
    it("derives main input queue from main_input.accepted", () => {
      const state = deriveRoomState([
        event(
          ...
          {
            input_id: "input_1",
            content: { text: "Hello agent", attachments: [] },
          },
          1,
          "user_1"
        ),
    ...
    it("updates input status through lifecycle events", () => {
      const events = [
        event(
          "main_input.accepted",
          { input_id: "input_1", content: { text: "Hello", attachments: [] } },
          1,
          "user_1"
        ),
    ...
    it("derives connector sync cursor from connector.snapshot.completed", () => {
      const state = deriveRoomState([
        event(
          ...
          { request_id: "snap_1", connector_id: "agent_1", last_sequence: 10 },
          1,
          "agent_1"
        ),
      ]);
      expect(state.connectorSyncCursor).toMatchObject({
