...
describe("API event parsing", () => {
  it("returns a valid event from websocket message data", () => {
    expect(parseCacpEventMessage(JSON.stringify(validEvent))).toEqual(
      validEvent
    );
  });
  ...
  it("returns undefined for JSON that is not a CACP event", () => {
    expect(
      parseCacpEventMessage(JSON.stringify({ hello: "world" }))
    ).toBeUndefined();
  });

  it("points generated local adapter commands at the API server when running through Vite dev server", () => {
    expect(pairingServerUrlFor("http://127.0.0.1:5173")).toBe(
      "http://127.0.0.1:3737"
    );
    expect(pairingServerUrlFor("http://localhost:5173")).toBe(
      "http://localhost:3737"
    );
    expect(pairingServerUrlFor("https://cacp.example.com")).toBe(
      "https://cacp.example.com"
    );
  });
...
describe("room API", () => {
  ...
  it("sends attachment IDs with a structured main input request", async () => {
    mockJsonResponse({ input_id: "input_1", status: "triggered" });
    ...
    await sendMainInput(session, "Inspect this", ["att_1"]);

    expect(fetch).toHaveBeenCalledWith("/rooms/room_1/main-inputs", {
      method: "POST",
      headers: {
  ...
  it("uploads a file without overriding the multipart content type", async () => {
    const attachment = {
      attachment_id: "att_1",
      name: "notes.txt",
    ...
    await expect(uploadAttachment(session, file)).resolves.toEqual(attachment);
  ...
  it("surfaces attachment upload failures", async () => {
    mockErrorResponse("unsupported_attachment");
    const session: RoomSession = {
      room_id: "room_1",
      participant_id: "owner_1",
      token: "owner-token",
    ...
    await expect(uploadAttachment(session, file)).rejects.toThrow(
      "unsupported_attachment"
    );
  });

  it("downloads authenticated attachment blobs using an encoded ID", async () => {
    ...
    await expect(
      fetchAttachmentBlob(session, "att/with space")
    ).resolves.toEqual(expect.any(Blob));
    expect(fetch).toHaveBeenCalledWith(
      "/rooms/room_1/attachments/att%2Fwith%20space",
      { headers: { authorization: "Bearer owner-token" } }
    );
  });

  it("surfaces attachment download failures", async () => {
    mockErrorResponse("attachment_not_found");
    const session: RoomSession = {
      room_id: "room_1",
      participant_id: "owner_1",
      token: "owner-token",
    ...
    await expect(fetchAttachmentBlob(session, "att_missing")).rejects.toThrow(
  ...
  it("creates a join request and polls its status", async () => {
    ...
    await expect(
      createJoinRequest("room_1", "invite_secret", "Member")
    ).resolves.toEqual({
      request_id: "req_1",
      request_token: "req_secret",
      status: "pending",
    ...
    await expect(
      joinRequestStatus("room_1", "req_1", "req_secret")
    ).resolves.toEqual({
      status: "approved",
      participant_id: "user_2",
      participant_token: "member_secret",
  ...
  it("starts local agent launches through the local pairing endpoint", async () => {
    ...
    await expect(
      createLocalAgentLaunch(session, {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "/rooms/room_1/agent-pairings/start-local",
      {
        method: "POST",
        headers: {
          "content-type": "application/json",
  ...
  it("creates a room and starts the configured local agent in one setup flow", async () => {
    ...
    await expect(
      createRoomWithLocalAgent("Planning", "Owner", {
        agent_type: "claude-code",
  ...
  it("posts participant activity requests", async () => {
    ...
    expect(fetch).toHaveBeenLastCalledWith(
      "/rooms/room_1/activity/typing/start",
      {
        method: "POST",
        headers: {
          "content-type": "application/json",
    ...
    expect(fetch).toHaveBeenLastCalledWith(
      "/rooms/room_1/activity/typing/stop",
      {
        method: "POST",
        headers: {
          "content-type": "application/json",
  ...
  it("posts connector snapshot requests to the plural snapshot endpoint with a cursor", async () => {
    ...
    await expect(requestConnectorSnapshot(session, 7)).resolves.toEqual({
      request_id: "snap_1",
    });
...
describe("invite URL", () => {
  it("generates /join path with room and token query params", () => {
    ...
    expect(url).toBe(
      "https://cacp.example.com/join?room=room_abc&token=invite_xyz"
    );
  });
});
