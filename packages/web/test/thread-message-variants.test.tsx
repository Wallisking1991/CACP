...
describe("Thread message variants", () => {
  it("distinguishes own, other human, AI, and system messages", () => {
    renderThread();
    expect(screen.getByText("My note").closest("article")).toHaveClass(
      "message-own"
    );
    expect(screen.getByText("Other note").closest("article")).toHaveClass(
      "message-human-other"
    );
    expect(screen.getByText("AI answer").closest("article")).toHaveClass(
      "message-ai-card"
    );
    expect(screen.getByText("History cleared").closest("article")).toHaveClass(
