...
describe("Thread run trace", () => {
  it("renders a running run trace as one card with the live answer and no legacy stream bubble", () => {
    ...
    expect(
      screen.queryByText("Legacy streaming answer")
    ).not.toBeInTheDocument();
    expect(document.querySelectorAll("article.message-ai-card")).toHaveLength(
      1
    );
  });
  ...
  it("renders a completed run trace answer in the run card and suppresses the duplicate final message", () => {
          ...
          nodes: [
            {
              ...activeRun.nodes[0],
              status: "completed",
              summary: "Read guidance",
              completed_at: "2026-05-06T00:00:04.000Z",
            },
          ],
        },
      ],
    ...
    expect(
      document.querySelector(".agent-run-card__process")
    ).toBeInTheDocument();
    expect(document.querySelectorAll("article.message-ai-card")).toHaveLength(
      1
    );
  ...
  it("shows the work process expanded while the run is active", () => {
      ...
      agentRuns: [
        {
          ...
          nodes: [
            ...
            {
              ...activeRun.nodes[0],
              title: "Search files: src/**/*.ts",
              detail: { elapsed_time_seconds: 2 },
            },
          ],
    ...
    const process = document.querySelector(
      "details.agent-run-card__process"
    ...
    expect(
      processScope.getByText("I should inspect the directory first.")
    ).toBeVisible();
  ...
  it("shows the final answer first and collapses the work process after completion", () => {
    ...
    expect(process).not.toBeNull();
    expect(
      answer.compareDocumentPosition(process as Node) &
        Node.DOCUMENT_POSITION_FOLLOWING
    ).toBeTruthy();
    expect(process?.open).toBe(false);
    expect(
      within(process as HTMLElement).getByText(/Work process/)
    ).toBeVisible();
  ...
  it("previews long node output and lets the user expand that node only", () => {
    const longOutput = Array.from(
      { length: 80 },
      (_, index) => `line ${index + 1}: detailed tool output`
    ).join("\n");
    renderThread({
      agentRuns: [
        {
    ...
    expect(
      screen.getByText(/line 80: detailed tool output/)
    ).toBeInTheDocument();
    expect(
      document.querySelector(".agent-run-node__text-preview--collapsed")
    ).toBeInTheDocument();
    ...
    expect(
      document.querySelector(".agent-run-node__text-preview--expanded")
    ).toBeInTheDocument();
  });
});
