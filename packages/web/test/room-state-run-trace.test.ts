...
describe("room state run trace projection", () => {
  it("merges streamed and final agent output into the run instead of legacy message views", () => {
    const state = deriveRoomState([
      ...
      event(
        "agent.output.delta",
        { turn_id: "turn_1", agent_id: "agent_1", chunk: "Partial " },
        3
      ),
      event(
        "agent.output.delta",
        { turn_id: "turn_1", agent_id: "agent_1", chunk: "answer" },
        4
      ),
      event(
      ...
      event(
        ...
        {
          message_id: "msg_1",
          content: { text: "Final answer", attachments: [] },
          kind: "agent",
          turn_id: "turn_1",
        },
        6
    ...
    expect(
      state.messages.find((message) => message.message_id === "msg_1")
    ).toBeUndefined();
    expect(
      state.streamingTurns.find((turn) => turn.turn_id === "turn_1")
    ).toBeUndefined();
  });
  ...
  it("keeps a running run trace answer out of legacy streaming turns", () => {
    const state = deriveRoomState([
      ...
      event(
        "agent.output.delta",
        { turn_id: "turn_1", agent_id: "agent_1", chunk: "Live answer" },
        3
      ),
    ]);
  ...
  it("marks agents as working while a run is active", () => {
    ...
    expect(
      state.avatarStatuses.find((avatar) => avatar.id === "agent_1")
    ).toMatchObject({
      status: "working",
      active: true,
    });
  ...
  it("does not create a duplicate failed message when a run trace already represents the failed turn", () => {
    ...
    expect(
      state.messages.find((message) => message.message_id === "failed-turn_1")
    ).toBeUndefined();
    expect(
      state.avatarStatuses.find((avatar) => avatar.id === "agent_1")
    ).toMatchObject({
