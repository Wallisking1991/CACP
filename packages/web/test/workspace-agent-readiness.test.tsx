...
function renderWorkspace(events: CacpEvent[], role = "owner" as const) {
  return render(
    <LangProvider>
      <Workspace
        session={{
...
describe("Workspace agent readiness UI", () => {
  ...
  it("shows waiting banner and disables composer when no agent", () => {
    ...
    expect(screen.getByTestId("agent-status-banner")).toHaveAttribute("data-status", "no_agent");
    expect(screen.getByPlaceholderText(/Type a message for the Agent/i)).toBeDisabled();
  });
  ...
  it("does not show fullscreen modal when session selection is needed", () => {
    ...
    renderWorkspace(events);
    expect(screen.queryByRole("dialog", { name: /Select Agent Session/i })).not.toBeInTheDocument();
  });
});
