...
describe("OrbitToggleTab", () => {
  it("renders as an accessible toggle with capped unread count", () => {
    ...
    render(
      <LangProvider>
        <OrbitToggleTab open={false} unreadCount={12} onClick={onClick} />
      </LangProvider>
    );
  ...
  it("hides the unread badge when count is zero", () => {
    render(
      <LangProvider>
        <OrbitToggleTab open={false} unreadCount={0} onClick={vi.fn()} />
      </LangProvider>
    );
  ...
  it("reflects open state via aria-pressed", () => {
    const { rerender } = render(
      <LangProvider>
        <OrbitToggleTab open={true} unreadCount={0} onClick={vi.fn()} />
      </LangProvider>
    );
  ...
  it("does not add mention class when hasMentions is false", () => {
    ...
    const badge = document.querySelector(".orbit-unread-badge");
    expect(badge).not.toBeNull();
    expect(badge!.classList.contains("orbit-unread-badge--mention")).toBe(
      false
    );
  });
});
