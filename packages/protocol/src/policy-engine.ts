...
function count(
  ...
  const eligible = participants.filter(
    (participant) =>
      canVote(participant) && (!roles || roles.includes(participant.role))
  );
  const latest = eligible
    .map((participant) => latestVote(votes, participant.id))
...
export function evaluatePolicy(
  ...
  if (
    policy.expires_at &&
    new Date(policy.expires_at).getTime() <= now.getTime()
  ) {
    return {
      status: "expired",
      ...
      approvals: 0,
      rejections: 0,
      eligible_voters: participants.filter(canVote).length,
    };
  }
  ...
  if (policy.type === "owner_approval") {
    ...
    if (result.approvals >= 1)
      return {
        ...
        eligible_voters: result.eligible.length,
      };
  ...
  if (policy.type === "majority") {
    ...
    return {
      status: "pending",
      reason: `waiting for ${required} approvals`,
      approvals: result.approvals,
      rejections: result.rejections,
      eligible_voters: result.eligible.length,
    };
  ...
  if (policy.type === "role_quorum") {
    ...
    if (result.approvals >= policy.min_approvals)
      return {
        status: "approved",
        reason: "role quorum reached",
        approvals: result.approvals,
        rejections: result.rejections,
        eligible_voters: result.eligible.length,
      };
    return {
      status: "pending",
      reason: `waiting for ${policy.min_approvals} role approvals`,
      approvals: result.approvals,
      rejections: result.rejections,
      eligible_voters: result.eligible.length,
    };
  ...
  if (result.rejections > 0)
    return {
      status: "rejected",
      reason: "unanimous policy received a rejection",
      approvals: result.approvals,
      rejections: result.rejections,
      eligible_voters: result.eligible.length,
    };
  if (result.eligible.length > 0 && result.approvals === result.eligible.length)
    return {
      status: "approved",
      reason: "all eligible voters approved",
      approvals: result.approvals,
      rejections: result.rejections,
      eligible_voters: result.eligible.length,
    };
  return {
    status: "pending",
    reason: "waiting for unanimous approval",
    approvals: result.approvals,
    rejections: result.rejections,
    eligible_voters: result.eligible.length,
  };
