...
import {
  ...
  AgentTurnRequestedPayloadSchema,
  AttachmentRefSchema,
  CacpEventSchema,
...
describe("CACP event schema", () => {
  ...
  it("accepts Claude session catalog events", () => {
    ...
    expect(ClaudeSessionCatalogUpdatedPayloadSchema.parse(payload)).toEqual(
      payload
    );
    expect(
      CacpEventSchema.parse({
    ...
    ).toBe("claude.session_catalog.updated");
  });
      ...
      sessions: [
        {
          ...
          importable: true,
          messages: [
            { author_role: "user", text: "should not be catalog metadata" },
          ],
        },
      ],
    });
  ...
  it("accepts Claude session selection events", () => {
    expect(
      ClaudeSessionSelectedPayloadSchema.parse({
        agent_id: "agent_1",
        mode: "resume",
        session_id: "session_1",
        selected_by: "owner_1",
  ...
  it("accepts Claude session ready payloads after connector startup", () => {
    expect(
      ClaudeSessionReadyPayloadSchema.parse({
        agent_id: "agent_1",
        mode: "fresh",
  ...
  it("accepts imported Claude transcript message payloads", () => {
    ...
    expect(ClaudeSessionImportMessagePayloadSchema.parse(payload)).toEqual(
      payload
    );
  });
  ...
  it("limits a single Claude session import to a bounded number of visible messages", () => {
    ...
    expect(() =>
      ClaudeSessionImportStartedPayloadSchema.parse(payload)
    ).toThrow();
  });
  ...
  it("accepts run node kinds and statuses", () => {
    expect(AgentRunNodeKindSchema.parse("reasoning_summary")).toBe(
      "reasoning_summary"
    );
    expect(AgentRunNodeKindSchema.parse("approval")).toBe("approval");
    expect(AgentRunNodeStatusSchema.parse("waiting_input")).toBe(
      "waiting_input"
    );
  ...
  it("accepts run lifecycle and interaction payloads", () => {
    expect(
      AgentRunStartedPayloadSchema.parse({
        run_id: "turn_1",
        turn_id: "turn_1",
        agent_id: "agent_1",
    ...
    expect(
      AgentRunNodeStartedPayloadSchema.parse({
        run_id: "turn_1",
        turn_id: "turn_1",
        agent_id: "agent_1",
        provider: "claude-code",
    ...
    expect(
      AgentRunApprovalRequestBodySchema.parse({
        agent_id: "agent_1",
        turn_id: "turn_1",
    ...
    expect(
      AgentRunElicitationResolveBodySchema.parse({
        action: "accept",
        content: { token: "abc" },
      })
    ).toEqual({
  ...
  it("enforces run node lifecycle and interaction invariants", () => {
    ...
    expect(
      AgentRunNodeUpdatedPayloadSchema.parse({
        run_id: "turn_1",
        turn_id: "turn_1",
        agent_id: "agent_1",
        provider: "claude-code",
    ...
    expect(
      AgentRunSourceRefsSchema.parse({
        tool_use_id: "toolu_1",
    ...
    expect(
      AgentRunApprovalResolveBodySchema.parse({
        decision: "allow",
      })
    ...
    expect(
      AgentRunElicitationRequestBodySchema.parse({
        agent_id: "agent_1",
        turn_id: "turn_1",
  ...
  it("accepts provider-neutral local agent session catalog events", () => {
    ...
    expect(AgentSessionCatalogUpdatedPayloadSchema.parse(payload)).toEqual(
      payload
    );
    expect(
      CacpEventSchema.parse({
    ...
    ).toBe("agent.session_catalog.updated");
  });

  it("accepts provider-neutral local agent import payloads and run-trace schemas", () => {
    expect(
      AgentSessionImportMessagePayloadSchema.parse({
        import_id: "import_1",
        agent_id: "agent_1",
        provider: "codex-cli",
        session_id: "019de11a-76d4-7ca3-96ea-27ad77a12187",
    ...
    expect(
      AgentRunFailedPayloadSchema.parse({
        run_id: "turn_1",
        turn_id: "turn_1",
        agent_id: "agent_1",
        provider: "codex-cli",
    ...
    expect(
      AgentRunNodeDeltaPayloadSchema.parse({
        run_id: "turn_1",
        turn_id: "turn_1",
        agent_id: "agent_1",
        provider: "codex-cli",
    ...
    expect(
      AgentRunNodeCompletedPayloadSchema.parse({
        run_id: "turn_1",
        turn_id: "turn_1",
        agent_id: "agent_1",
        provider: "codex-cli",
    ...
    expect(
      AgentRunNodeFailedPayloadSchema.parse({
        run_id: "turn_1",
        turn_id: "turn_1",
        agent_id: "agent_1",
        provider: "codex-cli",
...
describe("policy engine", () => {
  it("approves owner approval, majority, role quorum, and no approval policies", () => {
    expect(
      evaluatePolicy({ type: "owner_approval" }, participants, [
        { voter_id: "u_owner", vote: "approve" },
      ]).status
    ).toBe("approved");
    expect(
      evaluatePolicy({ type: "majority" }, participants, [
        { voter_id: "u_owner", vote: "approve" },
    ...
    expect(
      evaluatePolicy({ type: "no_approval" }, participants, []).status
    ).toBe("approved");
  });
  ...
  it("ignores votes from roles outside a role quorum policy", () => {
    const result = evaluatePolicy(
      { type: "role_quorum", required_roles: ["owner"], min_approvals: 1 },
      participants,
      [{ voter_id: "u_admin", vote: "approve" }]
    );
  ...
  it("rejects unanimous on rejection and expires old policies", () => {
    ...
    expect(
      evaluatePolicy({ type: "unanimous" }, participants, votes).status
    ).toBe("rejected");
    expect(
      evaluatePolicy(
        expired,
...
describe("orbit and connector event schemas", () => {
  ...
  it("accepts structured messages and attachment references", () => {
    const attachment = {
      attachment_id: "att_1",
      name: "diagram.png",
    ...
    expect(
      MessageCreatedPayloadSchema.parse({
        message_id: "message_1",
        content: { text: "Review this diagram", attachments: [attachment] },
        kind: "human",
        created_at: "2026-05-01T00:00:00.000Z",
      }).content.attachments
    ).toHaveLength(1);
    expect(
      AgentTurnRequestedPayloadSchema.parse({
        turn_id: "turn_1",
        agent_id: "agent_1",
    ...
    ).toBe("Review this diagram");
  });

  it("rejects legacy protocol versions and attachment-only messages", () => {
    expect(() =>
      CacpEventSchema.parse({
        protocol: "cacp",
        version: "0.2.0",
  ...
  it("accepts flat orbit note, clear, and quoted payloads", () => {
    ...
    expect(
      OrbitClearedPayloadSchema.parse({
        cleared_by: "user_1",
