...
describe("Claude runtime run-trace mapping", () => {
  it("streams SDK thinking deltas into a reasoning node and enriches streamed tool input", async () => {
      ...
      return createQuery([
        ...
        },
        {
          ...
          event: {
            ...
            delta: {
              type: "thinking_delta",
              thinking: "I should inspect the directory first. ",
            },
          },
        },
        {
          ...
          event: {
            ...
            delta: {
              type: "thinking_delta",
              thinking: "A glob search is enough.",
            },
          },
        },
        {
        ...
        },
        {
          ...
          event: {
            type: "content_block_delta",
            index: 1,
            delta: { type: "input_json_delta", partial_json: '"src/**/*.ts"' },
          },
        },
        {
          type: "stream_event",
        ...
        },
        {
          ...
          uuid: "tool_progress_1",
          session_id: "session_1",
        },
        {
          type: "tool_use_summary",
          summary: "Found TypeScript files",
          preceding_tool_use_ids: ["toolu_glob"],
          uuid: "tool_summary_1",
          session_id: "session_1",
        },
        {
    ...
    const reasoningNode = started.find(
      (node) => node.kind === "reasoning_summary" && node.title === "Thinking"
    );
    expect(reasoningNode).toBeDefined();
    const reasoningNodeId = reasoningNode?.node_id;
    expect(
      nodeDeltas
        .filter((delta) => delta.node_id === reasoningNodeId)
        .map((delta) => delta.chunk)
        .join("")
    ).toBe("I should inspect the directory first. A glob search is enough.");
    const completedReasoningNode = completed.find(
      (node) => node.node_id === reasoningNodeId
    );
    ...
    expect(
      updated.some(
        (node) =>
          node.node_id === "toolu_glob" &&
          node.title === "Search files: src/**/*.ts"
      )
    ).toBe(true);
    expect(
      updated.some(
        (node) =>
          node.node_id === "toolu_glob" &&
          (node.detail as Record<string, unknown> | undefined)?.tool_name ===
            "Glob"
      )
    ).toBe(true);
    expect(
      updated.some(
        (node) =>
          node.node_id === "toolu_glob" &&
          JSON.stringify(node.detail).includes("src/**/*.ts")
      )
    ).toBe(true);
    const latestGlobUpdate = updated
      .filter((node) => node.node_id === "toolu_glob")
      .at(-1);
    ...
    expect(
      completed.some(
        (node) =>
          node.node_id === "toolu_glob" &&
          node.summary === "Found TypeScript files"
      )
    ).toBe(true);
  ...
  it("preserves SDK result timing, cost, turns, and usage metadata", async () => {
    ...
    expect(result.usage?.model_usage).toEqual({
      "claude-sonnet-4-20250514": { inputTokens: 100, outputTokens: 50 },
    });
    expect(result.usage?.permission_denials).toEqual([
      {
        tool_name: "Bash",
        tool_use_id: "toolu_bash",
        tool_input: { command: "rm -rf dist" },
  ...
  it("streams long SDK-provided node summaries as full deltas while keeping terminal summaries short", async () => {
    ...
    const { runtime, nodeDeltas, completed } = createHarness(() =>
      createQuery([
        {
          type: "system",
          subtype: "init",
        ...
        },
        {
          type: "tool_use_summary",
          summary: longSummary,
          preceding_tool_use_ids: ["toolu_grep"],
          uuid: "tool_summary_1",
          session_id: "session_1",
        },
        createSuccessResult("session_1", "Done"),
    ...
    const completedTool = completed.find(
      (node) => node.node_id === "toolu_grep"
    );
    const summaryDeltas = nodeDeltas.filter(
      (delta) => delta.node_id === "toolu_grep"
    );
  ...
  it("routes Bash permission prompts through room-backed approval requests", async () => {
    ...
    const { runtime, approvals, started, updated } = createHarness(
      (_prompt, options) => {
    ...
    expect(
      started.some(
        (node) => node.node_id === "toolu_bash" && node.kind === "tool"
      )
    ).toBe(true);
    ...
    const latestBashUpdate = updated
      .filter((node) => node.node_id === "toolu_bash")
      .at(-1);
  ...
  it("reuses the first allowed Bash approval for later Bash tools in the same Claude runtime", async () => {
    ...
    const { runtime, setQueryImpl, approvals, updated } = createHarness(
      (_prompt, options) => {
        const canUseTool = options.canUseTool as
          | undefined
          | ((
    ...
    setQueryImpl((_prompt, options) => {
      ...
      return createQuery([
        ...
        {
          type: "tool_progress",
          tool_use_id: "toolu_bash_second",
          tool_name: "Bash",
          parent_tool_use_id: null,
          elapsed_time_seconds: 1,
          uuid: "tool_progress_2",
          session_id: "session_1",
        },
    ...
    expect(
      updated.some(
        (node) =>
          node.node_id === "toolu_bash_second" && node.status === "running"
      )
    ).toBe(true);
  });
  ...
  it("returns SDK-valid allow results for auto-allowed Write tools", async () => {
    ...
    const { runtime, approvals, updated } = createHarness(
      (_prompt, options) => {
        const canUseTool = options.canUseTool as
          | undefined
          | ((
    ...
    expect(
      updated.some(
        (node) => node.node_id === "toolu_write" && node.status === "running"
      )
    ).toBe(true);
  });
  ...
  it("auto-allows Bash in full access without room approval prompts", async () => {
    ...
    expect(
      started.some(
        (node) => node.node_id === "toolu_bash_full" && node.kind === "tool"
      )
    ).toBe(true);
    expect(
      updated.some(
        (node) =>
          node.node_id === "toolu_bash_full" && node.status === "running"
      )
    ).toBe(true);
  });

  it("preserves assistant tool_use input when progress updates arrive", async () => {
    const { runtime, started, updated } = createHarness(() =>
      createQuery([
        {
          type: "assistant",
          session_id: "session_1",
    ...
    expect(
      started.some(
        (node) =>
          node.node_id === "toolu_read" &&
          node.kind === "tool" &&
          node.title === "Read file: README.md"
      )
    ).toBe(true);
    const latestReadUpdate = updated
      .filter((node) => node.node_id === "toolu_read")
      .at(-1);
  ...
  it("attaches SDK tool_result content to the matching tool node output", async () => {
    ...
    expect(
      nodeDeltas
        .filter((delta) => delta.node_id === "toolu_search")
        .map((delta) => delta.chunk)
        .join("")
    ).toBe(toolResult);
    expect(completed.some((node) => node.node_id === "toolu_search")).toBe(
      true
    );
  });
  ...
  it("routes MCP elicitations through room-backed interaction requests", async () => {
    const { runtime, elicitations } = createHarness((_prompt, options) => {
      const onElicitation = options.onElicitation as
        | undefined
        | ((
            request: Record<string, unknown>,
            elicitationOptions: Record<string, unknown>
          ) => Promise<unknown>);
  ...
  it("maps memory recall, subagent forwarding, hook events, retries, and compaction into structured nodes", async () => {
    const { runtime, started, nodeDeltas, completed } = createHarness(() =>
      createQuery([
        {
          type: "system",
          subtype: "init",
        ...
        },
        {
          ...
          mode: "select",
          memories: [{ path: "D:/mem.md", scope: "personal" }],
          uuid: "memory_1",
          session_id: "session_1",
        },
        {
    ...
    expect(
      started.some(
        (node) => node.node_id === "task_1" && node.kind === "subagent"
      )
    ).toBe(true);
    const subagentMessage = started.find(
      (node) =>
        node.kind === "subagent_message" && node.parent_node_id === "task_1"
    );
    ...
    expect(
      nodeDeltas.some(
        (delta) =>
          delta.node_id === subagentMessage?.node_id &&
          delta.chunk === "Subagent reply"
      )
    ).toBe(true);
    expect(
      started.some((node) => node.node_id === "hook_1" && node.kind === "hook")
    ).toBe(true);
    ...
    expect(
      nodeDeltas.some(
        (delta) =>
          delta.node_id === "task_1" && delta.chunk === "Subagent complete"
      )
    ).toBe(true);
    expect(
      nodeDeltas.some(
        (delta) =>
          delta.node_id === "hook_1" &&
          delta.delta_type === "stdout" &&
          delta.chunk === "ok"
      )
    ).toBe(true);
    expect(
      completed.some(
        (node) =>
          node.node_id === "task_1" && node.summary === "Subagent complete"
      )
    ).toBe(true);
