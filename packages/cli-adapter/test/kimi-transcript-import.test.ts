...
} from "../src/kimi/transcript-import.js";
  ...
  return {
    createSession: () => ({
      ...
      prompt: () => ({
        ...
        respondQuestion: () => Promise.resolve(),
...
describe("buildKimiImportFromSessionEvents", () => {
  it("returns empty messages when no events", async () => {
    ...
    const result = await buildKimiImportFromSessionEvents({
      sdk,
      agentId: "agent_1",
      workingDir: "/p",
      sessionId: "s1",
  ...
  it("imports ContentPart text as assistant message", async () => {
    const sdk = mockKimiSdk([
      {
        type: "ContentPart",
        payload: { type: "text", text: "Sure, I can help." },
      },
    ]);
    const result = await buildKimiImportFromSessionEvents({
      sdk,
  ...
  it("ignores unknown event types", async () => {
    const sdk = mockKimiSdk([
      ...
      {
        type: "StatusUpdate",
        payload: { token_usage: { input_other: 10, output: 20 } },
      },
