...
} from "../src/codex/transcript-import.js";
...
describe("Codex transcript import", () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        [
          ...
          JSON.stringify({
            timestamp: "2026-05-01T01:17:02.827Z",
            type: "response_item",
            payload: {
              type: "function_call",
              name: "shell_command",
              arguments:
                '{"command":"Get-ChildItem -Force","workdir":"D:\\\\Development\\\\2","timeout_ms":120000}',
              call_id: "call_1",
            },
          }),
          JSON.stringify({
            timestamp: "2026-05-01T01:17:10.680Z",
            type: "response_item",
            payload: {
      ...
      expect(result.messages.map((message) => message.author_role)).toEqual([
        "user",
        "assistant",
        "command",
        "tool",
      ]);
      expect(result.messages.map((message) => message.source_kind)).toEqual([
        "user",
        "assistant",
        "command",
        "tool_result",
    ...
    try {
      ...
      writeFileSync(
        ...
        [
          JSON.stringify({
            type: "session_meta",
            payload: { id: "session_long", cwd: "D:\\Development\\2" },
          }),
          JSON.stringify({
            timestamp: "2026-05-01T01:15:10.727Z",
            type: "response_item",
            payload: {
              type: "message",
              role: "assistant",
              content: [{ type: "output_text", text: longAnswer }],
            },
          }),
        ].join("\n"),
        "utf8"
      ...
      expect(result.messages.map((message) => message.text.length)).toEqual([
        20000, 1,
      ]);
      expect(result.messages.map((message) => message.sequence)).toEqual([
        0, 1,
      ]);
      expect(result.messages.map((message) => message.part_index)).toEqual([
        0, 1,
      ]);
      expect(result.messages.map((message) => message.part_count)).toEqual([
        2, 2,
      ]);
    } finally {
      rmSync(root, { recursive: true, force: true });
  ...
  it("finds Codex session files by filename before reading metadata", async () => {
    ...
    try {
      ...
      const filePath = join(
        sessionsDir,
        "rollout-session_filename_match.jsonl"
      );
  ...
  it("does not find a Codex session outside the connector working directory", async () => {
    ...
    try {
      ...
      mkdirSync(sessionsDir, { recursive: true });
      writeFileSync(
        join(sessionsDir, "rollout-session_private_project.jsonl"),
        JSON.stringify({
          type: "session_meta",
