...
} from "../src/claude/transcript-import.js";
...
describe("Claude transcript import", () => {
    ...
    const sdk = {
      getSessionMessages: async (
      ...
      ) => [
        {
          uuid: "u1",
          type: "user",
          message: { content: "Please inspect the repo" },
        },
        {
          uuid: "a1",
          type: "assistant",
            ...
            content: [
              {
                ...
                id: "toolu_2",
                name: "Bash",
                input: { command: "corepack pnpm check" },
              },
            ],
          },
        },
  ...
  it("chunks imported messages into bounded upload batches", () => {
    ...
    expect(
      chunkClaudeImportMessages(messages, 50).map((chunk) => chunk.length)
    ).toEqual([50, 5]);
  });
  ...
  it("requests system messages and imports safe system metadata", async () => {
          ...
          return [
            {
              uuid: "sys1",
              type: "system",
              message: { content: "Compacted previous conversation" },
            },
          ];
        },
      },
    ...
    expect(calls).toEqual([
      { dir: "D:\\Development\\2", includeSystemMessages: true },
    ]);
    expect(result.messages).toHaveLength(1);
    expect(result.messages[0]).toMatchObject({
      author_role: "system",
      source_kind: "system",
      text: "Compacted previous conversation",
    });
  });
});
