...
import {
  createClaudeSdkFromModule,
  findClaudeBinary,
  loadClaudeSdk,
} from "../src/claude/claude-sdk.js";
...
describe("Claude SDK boundary", () => {
  it("normalizes query() behind local interfaces and forwards source-true options", async () => {
    ...
    const module = {
      ...
      getSessionMessages: async (
        _sessionId: string,
        _input: { dir: string }
      ) => [{ uuid: "m1", type: "assistant", message: "hello" }],
    };
  ...
  it("keeps listSessions and getSessionMessages available for catalog and import flows", async () => {
    ...
    await expect(
      sdk.getSessionMessages("session_1", {
        dir: ".",
  ...
  it("loadClaudeSdk forwards pathToClaudeCodeExecutable to the SDK wrapper", async () => {
    ...
    const sdk = await loadClaudeSdk({
      pathToClaudeCodeExecutable: "D:\\tools\\claude.exe",
    });
    sdk.query({ prompt: "test", options: { cwd: ".", model: "test" } });

    expect(queryCalls[0]?.options?.pathToClaudeCodeExecutable).toBe(
      "D:\\tools\\claude.exe"
    );
