...
function toolSummaryFromToolCall(
  event: KimiSdkStreamEvent
): string | undefined {
...
function toolResultFromToolResult(
  event: KimiSdkStreamEvent
): string | undefined {
...
export async function buildKimiImportFromSessionEvents(input: {
  ...
  const events = await sdk.parseSessionEvents(
    input.workingDir,
    input.sessionId
  );
  ...
  for (const event of events) {
    ...
    chunks.forEach((chunk, index) => {
      messages.push({
        ...
        provider: "kimi-cli",
        ...(chunks.length > 1
          ? { part_index: index, part_count: chunks.length, truncated: false }
          : {}),
      });
    });
  }
...
export function chunkKimiImportMessages(
  messages: AgentSessionImportMessagePayload[],
  size = 50
