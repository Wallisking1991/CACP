...
function wrapTurn(rawTurn: unknown): KimiSdkTurn {
  ...
  return {
    ...
    > {
      return (
        iterator as () => AsyncIterator<unknown, unknown, undefined>
      ).call(turn) as AsyncIterator<
        KimiSdkStreamEvent,
        {
          status: "finished" | "cancelled" | "max_steps_reached";
          steps?: number;
    ...
    async interrupt(): Promise<void> {
      if (typeof interrupt === "function")
        await (interrupt as () => Promise<void>).call(turn);
    },
    async approve(
      requestId: string,
      ...
      if (typeof approve === "function")
        await (
          approve as (requestId: string, response: string) => Promise<void>
        ).call(turn, requestId, response);
    },
    async respondQuestion(
      ...
      if (typeof respondQuestion === "function")
        await (
          respondQuestion as (
            rpcRequestId: string,
            questionRequestId: string,
        ...
        ).call(turn, rpcRequestId, questionRequestId, answers);
    },
    get result(): Promise<{
      status: "finished" | "cancelled" | "max_steps_reached";
      steps?: number;
...
function wrapSession(rawSession: unknown): KimiSdkSession {
  ...
  return {
    ...
    get executable(): string {
      return typeof session.executable === "string"
        ? session.executable
        : "kimi";
    },
    ...
    prompt(content: string | unknown[]): KimiSdkTurn {
      const raw = (prompt as (content: string | unknown[]) => unknown).call(
        session,
        content
      );
      return wrapTurn(raw);
    ...
    async close(): Promise<void> {
      if (typeof close === "function")
        await (close as () => Promise<void>).call(session);
    },
  };
}
...
export function createKimiSdkFromModule(module: UnknownSdkModule): KimiSdk {
  ...
  if (typeof createSessionFn !== "function") {
    throw new Error(
      "Kimi SDK createSession was not found. Install @moonshot-ai/kimi-agent-sdk."
    );
  }
  ...
  return {
    ...
    async listSessions(workDir): Promise<
      Array<{
        id: string;
        workDir: string;
        contextFile: string;
    ...
    > {
      const result = await (
        listSessionsFn as (workDir: string) => Promise<unknown>
      )(workDir);
      ...
      return result.map((item) => {
        ...
        return {
          ...
          contextFile:
            typeof record.contextFile === "string" ? record.contextFile : "",
          updatedAt:
...
export async function loadKimiSdk(): Promise<KimiSdk> {
  ...
  const module =
    (await import("@moonshot-ai/kimi-agent-sdk")) as UnknownSdkModule;
