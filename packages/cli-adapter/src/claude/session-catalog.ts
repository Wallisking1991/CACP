...
export function normalizeClaudeSession(
  session: ClaudeSdkSessionSummary,
  workingDir: string
  ...
  return {
    ...
    message_count: 0,
    byte_size:
      typeof session.fileSize === "number" ? Math.max(0, session.fileSize) : 0,
    importable: true,
  };
}

export async function listClaudeSessions(
  input: ClaudeSessionCatalogInput & {
    sdk?: Pick<ClaudeSdk, "listSessions" | "getSessionMessages">;
  }
  ...
  for (const raw of rawSessions) {
    ...
    try {
      const messages = await sdk.getSessionMessages(normalized.session_id, {
